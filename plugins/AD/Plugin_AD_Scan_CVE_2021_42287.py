...
class S4U2SELF:
    def __init__(
        self, behalfUser, username="", password="", domain="", hashes=None, dc_ip=None
    ):
    ...
    def printPac(self, data):
        ...
        for bufferN in range(pacType["cBuffers"]):
            ...
            if infoBuffer["ulType"] == 1:
                ...
                # kerbdata.dump()
                output.debug(
                    f"Domain SID:{kerbdata['LogonDomainId'].formatCanonical()}\n"
                )
    ...
    def dump(self):
        ...
        clientName = Principal(
            self.__behalfUser, type=constants.PrincipalNameType.NT_PRINCIPAL.value
        )

        S4UByteArray = struct.pack("<I", constants.PrincipalNameType.NT_PRINCIPAL.value)
        # S4UByteArray += b(self.__behalfUser) + b(self.__domain) + b'Kerberos'
        S4UByteArray += (
            self.__behalfUser.encode("utf-8")
        ...
        if self.__nthash != "" and (
            isinstance(self.__nthash, bytes) and self.__nthash != b""
        ):
            ...
            if newCipher.enctype == Enctype.RC4:
                ...
                key = newCipher.string_to_key(
                    self.__password, self.__domain.upper() + self.__username, None
                )
