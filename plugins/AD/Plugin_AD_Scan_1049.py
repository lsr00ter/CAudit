...
class PluginADNSPIAccess(PluginADScanBase):
    ...
    def run_script(self, args) -> dict:
        ...
        query = f"(distinguishedName=CN=Directory Service,CN=Windows NT,CN=Services,{search_base})"  # 也可以用下面这种直接找 DSHeuristics，然后再在下面判定字符串 CN=Directory Service,CN=Windows NT,CN=Services,
        # query = "(DSHeuristics=*)
        ...
        for entry in entry_generator:
            ...
            #     "CN=Directory Service,CN=Windows NT,CN=Services,")            #这句删掉了，因为查询语句决定了结果的 DN 必然包含这个字段
            str1 = entry["attributes"]["DSHeuristics"]

            # if ret != -1 and len(entry["attributes"]["DSHeuristics"]) >= 8 and str(
            #         str1[7:8]) != "0":
            if (
                len(entry["attributes"]["DSHeuristics"]) >= 8 and str(str1[7:8]) != "0"
            ):  # 因为没有 ret 这个地方也不需要判定
                result["status"] = 1
                instance = {}
