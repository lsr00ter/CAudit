    ...
    def run_script(self, args) -> dict:
        ...
        for entry in entry_generator:
            ...
            if s1 & privilege_set == set():
                if (
                    "CN=HealthMailbox" not in s
                    and "SystemMailbox{" not in s
                    and "FederatedEmail." not in s
                    and "Migration" not in s
                    and "DiscoverySearchMailbox {D919BA05-46A6-415f-80AD-7E09334BB852}"
                    not in s
                    and "CN=Exchange Online-ApplicationAccount" not in s
                ):
                    result["status"] = 1
                    instance = {}
