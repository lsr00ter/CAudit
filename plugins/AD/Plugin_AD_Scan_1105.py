...
class PluginADESC32(PluginADScanBase):
    ...
    def run_script(self, args) -> dict:
        ...
        instance_list = []
        vuln_templates, sids_resolver = certilib.checkECS32(
        ...
        for temp in vuln_templates:
            ...
            for ace in temp.dacl.aces:
                ...
                if (
                    ...
                    or mask.hasPriv(
                        certilib.ACCESS_ALLOWED_OBJECT_ACE.ADS_RIGHT_DS_WRITE_PROP
                    )
                ):
            ...
            if (
                len(enrollment_acl)
                == 0 & len(autoenrollment_acl)
                == 0 & len(writeowner_acl)
                == 0 & len(writedacl_acl)
                == 0 & len(writeprop_acl)
                == 0
            ):
                ...
                if (
                    temp.name == "User"
                    or temp.name == "Machine"
                    or temp.name == "DomainController"
                ):
                    continue
                else:
                    attacked = True
                    ...
                    instance["证书标志位"] = ", ".join(
                        temp.certificate_name_flags_names
                    )
