...
import traceback
from modules.adi_lib.ldap.acls import (
    SecurityDescriptor,
    ACE,
    ACCESS_MASK,
...
class PluginADIllegalGeneralPermissionsAcount(PluginADScanBase):
    ...
    def run_script(self, args) -> dict:
        ...
        attributes = ["cn", "nTSecurityDescriptor", "name"]
        ...
        try:
            for entry in entry_generator:
                ...
                sd = SecurityDescriptor(
                    BytesIO(entry["raw_attributes"].get("nTSecurityDescriptor")[0])
                )
                for ace_obj in sd.dacl.aces:
                    ...
                    if ace_obj.ace.AceType == 0x05:
                        ...
                        writeprivs = ace_obj.acedata.mask.has_priv(
                            ACCESS_MASK.ADS_RIGHT_DS_WRITE_PROP
                        )
                        if writeprivs:
                            if not ace_obj.acedata.has_flag(
                                ...
                                instance_list.append(instance)
                        # Extended rights
                        control_access = ace_obj.acedata.mask.has_priv(
            ...
            instance_list = [
                ...
                if not (
                    i.get("被授予权限用户").endswith("519")
                    or i.get("被授予权限用户").endswith("512")
                    or i.get("被授予权限用户").endswith("526")
                    or i.get("被授予权限用户").endswith("527")
                    or i.get("被授予权限用户").endswith("517")
                )
            ]
            ...
            for i in instance_list:
                ...
                for entry in entry_generator:
                    ...
                    if (
                        ...
                        in entry["attributes"]["distinguishedName"]
                    ):
                        id = i.get("被授予权限用户").split("-")[-1]
                        instance_list = [
                            info
                            for info in instance_list
                            if not (info.get("被授予权限用户").endswith(id[-1]))
                        ]
                        continue
