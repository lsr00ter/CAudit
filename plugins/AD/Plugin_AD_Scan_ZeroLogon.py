...
class PluginADZeroLogon(PluginADScanBase):
    ...
    def run_script(self, args) -> dict:
        try:
            dce = transport.DCERPCTransportFactory(
                f"ncacn_np:{self.dc_ip}[\\pipe\\netlogon]"
            ).get_dce_rpc()
            dce.connect()
            dce.set_credentials("", "")
        ...
        for attempt in range(0, MAX_ATTEMPTS):
            ...
            try:
                ...
                self.result["desc"] = (
                    f"[{self.dc_ip}] Netlogon Auth OK, successfully bypassed autentication using Zerologon after {attempt} attempts!"
                )
                break
