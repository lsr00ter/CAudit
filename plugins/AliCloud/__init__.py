...
class AliCloud:
    ...
    def listpoliciesforuser(self, username):
        ...
        try:
            ...
            return json.loads(UtilClient.to_jsonstring(TeaCore.to_map(response)))[
                "body"
            ]["Policies"]["Policy"]
        except Exception as error:
            output.error(error.message)
    ...
    def listbucketobjects(self, region, bucket_name):
        ...
        for o in object_results.object_list:
            ...
            if size == "0.00B":
                ...
                results.append(
                    [o.key, size, datetime.fromtimestamp(o.last_modified), key_url]
                )
    ...
    def listrds(self, region_id):
        ...
        try:
            result_lists = []
            # 复制代码运行请自行打印 API 的返回值
            resp = client.describe_dbinstances_with_options(
                describe_dbinstances_request, runtime
            )
    ...
    def executecommand(self, instance_id, command, region_id, os_type="linux"):
        ...
        try:
            resp = client.create_command_with_options(
                ecs_20140526_models.CreateCommandRequest(
                    region_id=region_id,
                    name="testName",
                    type=instance_type,
        ...
        try:
            ...
            resp = client.invoke_command_with_options(
                ecs_20140526_models.InvokeCommandRequest(
                    region_id=region_id,
        ...
        try:
            ...
            while not get_results:
                ...
                for result in resp.body.invocation.invocation_results.invocation_result:
                    if (
                        result.invoke_record_status == "Finished"
                        and execute_id == result.invoke_id
                    ):
        ...
        try:
            # 复制代码运行请自行打印 API 的返回值
            resp = client.delete_command_with_options(
                ecs_20140526_models.DeleteCommandRequest(
                    region_id=region_id, command_id=command_id
                ),
                runtime,
            )
