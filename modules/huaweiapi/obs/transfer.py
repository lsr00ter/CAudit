...
def _resume_download(
    ...
    down_operation = downloadOperation(
        util.to_string(bucketName),
        util.to_string(objectKey),
        util.to_string(downloadFile),
        partSize,
        taskNum,
        enableCheckPoint,
        util.to_string(checkPointFile),
...
def _resume_download_with_operation(down_operation):
    if down_operation.size == 0:
        ...
        with open(down_operation.fileName, "wb"):
            pass
        if down_operation.progressCallback is not None and callable(
            down_operation.progressCallback
        ):
...
class Operation(object):
    ...
    def _delete_record(self):
        if os.path.exists(self.checkPointFile):
            ...
            self.obsClient.log_client.log(
                INFO,
                "delete checkpoint file success. path is:{0}".format(
                    self.checkPointFile
                ),
            )
    ...
    def _write_record(self, record):
        with open(self.checkPointFile, "w") as f:
            ...
            self.obsClient.log_client.log(
                INFO,
                "write checkpoint file success. file path is {0}".format(
                    self.checkPointFile
                ),
            )
...
class uploadOperation(Operation):
    def __init__(
        ...
        resp = self.obsClient.headBucket(
            self.bucketName, extensionHeaders=extensionHeaders
        )
        if resp.status > 300:
            raise Exception(
                "head bucket {0} failed. Please check. Status:{1}.".format(
                    self.bucketName, resp.status
                )
            )
    ...
    def _upload(self):
        ...
        try:
            if len(unfinished_upload_parts) > 0:
                ...
                thread_pools = _ThreadPool(
                    functools.partial(
                        self._produce, upload_parts=unfinished_upload_parts
                    ),
                ...
                if self._abort:
                    self.obsClient.abortMultipartUpload(
                        self.bucketName,
                        self.objectKey,
                ...
                for p in self._record["uploadParts"]:
                    if not p["isCompleted"]:
                        ...
                        raise Exception(
                            "some parts are failed when upload. Please try again"
                        )

            part_Etags = []
            for part in self._record["partEtags"]:
                part_Etags.append(
                    CompletePart(partNum=part["partNum"], etag=part["etag"])
                )
    ...
    def _load(self):
        ...
        if self._record and not (
            self._type_check(self._record) and self._check_upload_record(self._record)
        ):
            if (
                self._record["bucketName"]
                and self._record["objectKey"]
                and self._record["uploadId"] is not None
            ):
                self.obsClient.abortMultipartUpload(
                    self._record["bucketName"],
                    self._record["objectKey"],
                    self._record["uploadId"],
                    extensionHeaders=self.extensionHeaders,
                )
        ...
        try:
            for key in ("bucketName", "objectKey", "uploadId", "uploadFile"):
                if not isinstance(record[key], str):
                    self.obsClient.log_client.log(
                        ERROR,
                        "{0} is not a string type. {1} belong to {2}".format(
                            key, record[key], record[key].__class__
                        ),
                    )
                    return False
            if not isinstance(record["fileStatus"], list):
                self.obsClient.log_client.log(
                    ERROR,
                    "fileStatus is not a list.It is {0} type".format(
            ...
            if not isinstance(record["uploadParts"], list):
                self.obsClient.log_client.log(
                    ERROR,
                    "uploadParts is not a list.It is {0} type".format(
            ...
            if not isinstance(record["partEtags"], list):
                self.obsClient.log_client.log(
                    ERROR,
                    "partEtags is not a dict.It is {0} type".format(
    ...
    def _check_upload_record(self, record):
        if not (
            (record["bucketName"] == self.bucketName)
            and (record["objectKey"] == self.objectKey)
            and (record["uploadFile"] == self.fileName)
        ):
            self.obsClient.log_client.log(
                INFO,
        ...
        if record["uploadId"] is None:
            self.obsClient.log_client.log(
                INFO,
                "{0} (uploadId) not exist, clear the record.".format(
                    record["upload_id"]
                ),
            )
            return False
        ...
        if (
            record["fileStatus"][0] != self.size
            or record["fileStatus"][1] != self.lastModified
        ):
            self.obsClient.log_client.log(
                INFO, "{0} was changed, clear the record.".format(self.fileName)
            )
            return False
        if self.checkSum and len(record["fileStatus"]) >= 3:
            checkSum = util.base64_encode(
                util.md5_file_encode_by_size_offset(
                    file_path=self.fileName, size=self.size, offset=0
                )
            )
            if record["fileStatus"][2] and record["fileStatus"][2] != checkSum:
                self.obsClient.log_client.log(
                    INFO,
                    "{0} content was changed, clear the record.".format(self.fileName),
                )
                return False
        return True
    ...
    def _slice_file(self):
        ...
        if self.total_parts == 0:
            ...
            if self.size % self.partSize != 0:
                uploadParts[self.total_parts - 1].length = util.to_long(
    ...
    def get_init_upload_result(self):
        return self.obsClient.initiateMultipartUpload(
            self.bucketName,
            self.objectKey,
    ...
    def _prepare(self):
        ...
        if self.checkSum:
            fileStatus.append(
                util.base64_encode(
                    util.md5_file_encode_by_size_offset(self.fileName, self.size, 0)
                )
            )
        ...
        if resp.status > 300:
            raise Exception(
                "initiateMultipartUpload failed. ErrorCode:{0}. ErrorMessage:{1}".format(
                    resp.errorCode, resp.errorMessage
                )
            )
        ...
        self.obsClient.log_client.log(
            INFO, "prepare new upload task success. uploadId = {0}".format(uploadId)
        )
        if self.enableCheckPoint:
    ...
    def _upload_part(self, part):
        if not self._is_abort():
            try:
                ...
                if resp.status < 300:
                    self._record["uploadParts"][part["partNumber"] - 1][
                        "isCompleted"
                    ] = True
                    self._record["partEtags"].append(
                        CompletePart(util.to_int(part["partNumber"]), resp.body.etag)
                    )
                    if self.enableCheckPoint:
                        with self._lock:
                            self._write_record(self._record)
                ...
                self.obsClient.log_client.log(
                    DEBUG, "upload part %s error, %s" % (part["partNumber"], e)
                )
                self.obsClient.log_client.log(ERROR, traceback.format_exc())

    def get_upload_part_resp(self, part):
        return self.obsClient._uploadPartWithNotifier(
            self.bucketName,
            self.objectKey,
            part["partNumber"],
            self._record["uploadId"],
...
class downloadOperation(Operation):
    def __init__(
        ...
        super(downloadOperation, self).__init__(
            bucketName,
            objectKey,
            downloadFile,
            partSize,
        ...
        metadata_resp = self.obsClient.getObjectMetadata(
            self.bucketName,
            self.objectKey,
            self.versionId,
            extensionHeaders=self.extensionHeaders,
        ...
        if metadata_resp.status < 300:
            ...
            self.size = (
                metadata_resp.body.contentLength
                if metadata_resp.body.contentLength is not None
                and metadata_resp.body.contentLength >= 0
                else 0
            )
        else:
    ...
    def _download(self):
        ...
        try:
            if len(unfinished_down_parts) > 0:
                if (self.size - sent_bytes) > 0:
                    if self.progressCallback is not None:
                        self.notifier = progress.ProgressNotifier(
                            self.progressCallback, self.size
                        )
                        inner_notifier = True
                        self.notifier.start()
                    self.notifier.send(sent_bytes)
                ...
                thread_pools = _ThreadPool(
                    functools.partial(
                        self._produce, download_parts=unfinished_down_parts
                    ),
                    [self._consume] * self.taskNum,
                )
                thread_pools.run()
                ...
                for p in self._record["downloadParts"]:
                    if not p["isCompleted"]:
                        raise Exception(
                            "some parts are failed when download. Please try again"
                        )
    ...
    def _load(self):
        ...
        if self._record is not None and not (
            self._type_record(self._record)
            and self._check_download_record(self._record)
        ):
            self._delete_record()
            self._delete_tmp_file()
            self._record = None
    ...
    def _prepare(self):
        ...
        tmp_file_status = [
            os.path.getsize(self._tmp_file),
    ...
    def _type_record(self, record):
        try:
            ...
            if not isinstance(record["downloadParts"], list):
                self.obsClient.log_client.log(
                    ERROR,
                    "downloadParts is not a list.It is {0} type".format(
            ...
            if not isinstance(record["objectStatus"], list):
                self.obsClient.log_client.log(
                    ERROR,
                    "objectStatus is not a list.It is {0} type".format(
            ...
            if not isinstance(record["tmpFileStatus"], list):
                self.obsClient.log_client.log(
                    ERROR,
                    "tmpFileStatus is not a dict.It is {0} type".format(
    ...
    def _split_object(self):
        ...
        for i in range(1, num_counts + 1):
                ...
                start + util.to_long(self.partSize)
                if i != num_counts
                else util.to_long(self.size)
            )
    ...
    def _download_part(self, part):
        get_object_request = GetObjectRequest(
            versionId=self.versionId, imageProcess=self.imageProcess
        )
        get_object_header = self._copy_get_object_header(self.header)
        get_object_header.range = (
            util.to_string(part["offset"]) + "-" + util.to_string(part["length"])
        )
        if not self._is_abort():
            ...
            try:
                ...
                if resp.status < 300:
                    ...
                    self._record["downloadParts"][part["partNumber"] - 1][
                        "isCompleted"
                    ] = True
                    if self.enableCheckPoint:
                        with self._lock:
                    ...
                    self._exception += "response from server is something wrong. ErrorCode:{0}, ErrorMessage:{1}\n".format(
                        resp.errorCode, resp.errorMessage
                    )
                    self.obsClient.log_client.log(
                        ERROR,
                ...
                self.obsClient.log_client.log(
                    DEBUG, "download part %s error, %s" % (part["partNumber"], e)
                )
                self.obsClient.log_client.log(ERROR, traceback.format_exc())
            finally:
...
def _parse_string(content):
    if const.IS_PYTHON2:
        if isinstance(content, dict):
            return dict(
                [
                    (_parse_string(key), _parse_string(value))
...
class _ThreadPool(object):
    ...
    def run(self):
        self._add_and_run(
            self._threading_thread(target=self._producer_start), self._threads_producer
        )
        for thread in self._threads_producer:
            thread.join()
        for consumer in self._consumers:
            self._add_and_run(
                self._threading_thread(target=self._consumer_start, args=(consumer,)),
                self._threads_consumer,
            )
