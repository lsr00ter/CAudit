...
def get_readable_entity(
    readable, chunk_size=const.READ_ONCE_LENGTH, notifier=None, auto_close=True
):
    if notifier is None:
...
def get_readable_entity_by_total_count(
    readable,
    totalCount,
    ...
    return get_entity_for_send_with_total_count(
        readable, totalCount, chunk_size, notifier, auto_close
    )


def get_file_entity_by_total_count(
    file_path, totalCount, chunk_size=const.READ_ONCE_LENGTH, notifier=None
):
    f = open(file_path, "rb")
    return get_entity_for_send_with_total_count(f, totalCount, chunk_size, notifier)


def get_entity_for_send_with_total_count(
    readable,
    totalCount=None,
    chunk_size=const.READ_ONCE_LENGTH,
    notifier=None,
    ...
    def entity(conn):
        ...
        try:
            while True:
                ...
                if (totalCount is not None and readCount >= totalCount) or (
                    totalCount is not None and not chunk
                ):
...
def get_file_entity_by_offset_partsize(
    file_path, offset, totalCount, chunk_size=const.READ_ONCE_LENGTH, notifier=None
):
    f = open(file_path, "rb")
    f.seek(offset)
    return get_entity_for_send_with_total_count(f, totalCount, chunk_size, notifier)
...
def md5_file_encode_by_size_offset(
    file_path=None, size=None, offset=None, chuckSize=None
):
    if file_path is not None and size is not None and offset is not None:
        ...
        with open(file_path, "rb") as fp:
            ...
            while read_count < size:
                read_size = (
                    CHUNK_SIZE if size - read_count >= CHUNK_SIZE else size - read_count
                )
