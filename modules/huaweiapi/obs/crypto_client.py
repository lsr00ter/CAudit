...
from modules.huaweiapi.obs.transfer import (
    _resume_download_with_operation,
    downloadOperation,
    uploadOperation,
)
...
class CryptoObsClient(ObsClient):
    ...
    def initiateEncryptedMultipartUpload(
        self,
        bucketName,
        objectKey,
        crypto_cipher,
        ...
        resp = super(CryptoObsClient, self).initiateMultipartUpload(
            bucketName,
            objectKey,
            acl,
            storageClass,
    ...
    def putContent(
        ...
        if (const.IS_PYTHON2 and isinstance(content, unicode)) or isinstance(
            content, str
        ):
        ...
        put_result = super(CryptoObsClient, self).putContent(
            bucketName,
            objectKey,
            content,
            metadata,
    ...
    def uploadEncryptedPart(
        ...
        if isFile:
            ...
            content = crypto_cipher
        return super(CryptoObsClient, self).uploadPart(
            bucketName,
            objectKey,
            partNumber,
            uploadId,
    ...
    def uploadFile(
        ...
        upload_operation = EncryptedUploadOperation(
            self.cipher_generator,
    ...
    def downloadFile(
        ...
        down_operation = DecryptedDownloadOperation(
            self.cipher_generator,
            util.to_string(bucketName),
            util.to_string(objectKey),
    ...
    def _parse_content(
        ...
        if readable.status >= 300:
            return super(CryptoObsClient, self)._parse_content(
                objectKey,
                conn,
                readable,
        ...
        crypto_info = self.cipher_generator.get_crypto_info_from_headers(
            dict(readable.getheaders())
        )
        try:
            iv_offset = int(download_start.split("-")[0])
        except (AttributeError, ValueError):
            iv_offset = 0
        decryptedObject = self.cipher_generator.new(
            readable, is_decrypt=True, crypto_info=crypto_info
        )
    ...
    def _encrypted_upload_part(
        ...
        content = self.cipher_generator.new(
            open(checked_file_part_info["file_path"], "rb"), crypto_info=crypto_info
        )
        content.seek(checked_file_part_info["offset"])
        headers = dict()
        if self.cipher_generator.need_sha256:
            headers[self.ha.content_sha256_header()] = content.calculate_sha256(
                partSize
            )[1]
        return super(CryptoObsClient, self)._uploadPartWithNotifier(
            bucketName,
            objectKey,
            partNumber,
            uploadId,
    ...
    def _covert_string_to_bytes_io(str_object):
        if (
            const.IS_PYTHON2
            and isinstance(str_object, unicode)
...
class EncryptedUploadOperation(uploadOperation):
    def __init__(
        ...
        metadata = self.encrypted_content.gen_need_metadata_and_headers(
            metadata, headers
        )
        super(EncryptedUploadOperation, self).__init__(
            bucketName,
            objectKey,
            uploadFile,
            partSize,
    ...
    def _check_upload_record(self, record):
        ...
        if not self.cipher_generator.check_upload_record(
            self._record, self.encrypted_content.safe_crypto_info()
        ):
            self.obsClient.log_client.log(
                INFO, "The crypto_info was changed. clear the record"
            )
            return False
    ...
    def _load(self):
        ...
        if "crypto_mod" in self._record:
            self.encrypted_content = self.cipher_generator.new(
                "", crypto_info=self._record
            )
        else:
            self._record = self.encrypted_content.gen_need_record(self._record)
    ...
    def get_upload_part_resp(self, part):
        return self.obsClient._encrypted_upload_part(
            self.bucketName,
            self.objectKey,
            part["partNumber"],
...
class DecryptedDownloadOperation(downloadOperation):
    ...
    def _check_download_record(self, record):
        self._record = self._get_record()
        if not self.cipher_generator.check_download_record(
            self._record, self.decrypted_content.safe_crypto_info()
        ):
            self.obsClient.log_client.log(
                INFO, "The crypto_info was changed. clear the record"
            )
