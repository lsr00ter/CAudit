...
class Adapter(object):
    ...
    def _get_meta_header_prefix(self):
        return (
            const.OBS_META_HEADER_PREFIX if self.is_obs else const.V2_META_HEADER_PREFIX
        )
    ...
    def default_storage_class_header(self):
        return (
            self._get_header_prefix() + "storage-class"
            if self.is_obs
            else "x-default-storage-class"
        )
    ...
    def bucket_region_header(self):
        return (
            self._get_header_prefix() + "bucket-location"
            if self.is_obs
    ...
    def sse_kms_key_header(self):
        return (
            self._get_header_prefix() + "server-side-encryption-kms-key-id"
            if self.is_obs
    ...
    def copy_source_sse_c_key_header(self):
        return (
            self._get_header_prefix()
            + "copy-source-server-side-encryption-customer-key"
        )

    def copy_source_sse_c_key_md5_header(self):
        return (
            self._get_header_prefix()
            + "copy-source-server-side-encryption-customer-key-MD5"
        )
    ...
    def adapt_group(self, group):
        ...
        return (
            group
            if group in self.V2_ALLOWED_GROUP
            else (
                "http://acs.amazonaws.com/groups/global/AllUsers"
                if group == "Everyone"
                else (
    ...
    def _adapt_group_is_obs(self, group):
        return (
            group
            if group in self.OBS_ALLOWED_GROUP
            else (
                "Everyone"
                if group
                in ("http://acs.amazonaws.com/groups/global/AllUsers", "AllUsers")
                else None
            )
        )
    ...
    def adapt_event_type(self, eventType):
        if self.is_obs:
            return (
                eventType
                if eventType in self.OBS_ALLOWED_EVENT_TYPE
                else eventType[3:] if eventType in self.V2_ALLOWED_EVENT_TYPE else None
            )

        return (
            eventType
            if eventType in self.V2_ALLOWED_EVENT_TYPE
    ...
    def adapt_storage_class(self, storageClass):
        if self.is_obs:
            return (
                storageClass
                if storageClass in self.OBS_ALLOWED_STORAGE_CLASS
                else (
                    "WARM"
                    if storageClass == "STANDARD_IA"
                    else "COLD" if storageClass == "GLACIER" else None
                )
            )
        return (
            storageClass
            if storageClass in self.V2_ALLOWED_STORAGE_CLASS
            else (
                "STANDARD_IA"
                if storageClass == "WARM"
                else "GLACIER" if storageClass == "COLD" else None
            )
        )
...
class Convertor(object):
    ...
    def trans_create_bucket(self, **kwargs):
        ...
        if header is not None:
            self._put_key_value(
                headers,
                self.ha.acl_header(),
                self.ha.adapt_acl_control(header.get("aclControl")),
            )
            self._put_key_value(
                headers,
                self.ha.default_storage_class_header(),
                self.ha.adapt_storage_class(header.get("storageClass")),
            )
            self._put_key_value(
                headers, self.ha.az_redundancy_header(), header.get("availableZone")
            )
            ...
            if extensionGrants is not None and len(extensionGrants) > 0:
                ...
                for extensionGrant in extensionGrants:
                    permission = self.ha.adapt_extension_permission(
                        extensionGrant.get("permission")
                    )
                    if (
                        permission is not None
                        and extensionGrant.get("granteeId") is not None
                    ):
                        ...
                        granteeIds.add(
                            "id=%s" % util.to_string(extensionGrant["granteeId"])
                        )
        ...
        return {
            ...
            "entity": (
                ...
                else self.trans_bucket_location(kwargs.get("location"))
            ),
        }
    ...
    def trans_bucket_location(self, location):
        ...
        ET.SubElement(
            root, "Location" if self.is_obs else "LocationConstraint"
        ).text = util.to_string(location)
    ...
    def trans_list_buckets(self, **kwargs):
        ...
        if kwargs.get("bucketType"):
            self._put_key_value(
                headers, self.ha.queryPFS_header(), kwargs.get("bucketType")
            )
    ...
    def trans_list_versions(self, **kwargs):
        ...
        if version is not None:
            ...
            self._put_key_value(
                pathArgs, "version-id-marker", version.get("version_id_marker")
            )
    ...
    def trans_get_bucket_metadata(self, **kwargs):
        ...
        self._put_key_value(
            headers, const.ACCESS_CONTROL_REQUEST_HEADERS_HEADER, _requestHeaders
        )
        return {"headers": headers}
    ...
    def trans_set_bucket_tagging(self, **kwargs):
        ...
        return {
            ...
            "headers": {
                const.CONTENT_MD5_HEADER: util.base64_encode(util.md5_encode(entity))
            },
            "entity": entity,
        }
    ...
    def trans_cors_rules(corsRuleList):
        ...
        for cors in corsRuleList:
            ...
            if cors.get("maxAgeSecond") is not None:
                ET.SubElement(corsRuleEle, "MaxAgeSeconds").text = util.to_string(
                    cors["maxAgeSecond"]
                )
    ...
    def trans_delete_objects_request(self, deleteObjectsRequest):
        ...
        if deleteObjectsRequest is not None:
            if deleteObjectsRequest.get("quiet") is not None:
                ET.SubElement(root, "Quiet").text = util.to_string(
                    deleteObjectsRequest["quiet"]
                ).lower()
            if deleteObjectsRequest.get("encoding_type") is not None:
                ET.SubElement(root, "EncodingType").text = util.to_string(
                    deleteObjectsRequest["encoding_type"]
                )
                encoding_type = util.to_string(deleteObjectsRequest["encoding_type"])
            if (
                isinstance(deleteObjectsRequest.get("objects"), list)
                and len(deleteObjectsRequest["objects"]) > 0
            ):
                for obj in deleteObjectsRequest["objects"]:
                    if obj.get("key") is not None:
                        ...
                        ET.SubElement(objectEle, "Key").text = util.safe_decode(
                            key_text
                        )
                        if obj.get("versionId") is not None:
                            ET.SubElement(objectEle, "VersionId").text = (
                                util.safe_decode(obj["versionId"])
                            )
        return ET.tostring(root, "UTF-8")
    ...
    def _transTransition(self, ruleEle, transition):
        ...
        if transition.get("days") is not None:
            ET.SubElement(transitionEle, "Days").text = util.to_string(
                transition["days"]
            )
        elif transition.get("date") is not None:
            date = (
                transition["date"].ToUTMidTime()
                if isinstance(transition["date"], DateTime)
                else transition["date"]
            )
    ...
    def _transNoncurrentVersionTransition(self, ruleEle, noncurrentVersionTransition):
        noncurrentVersionTransitionEle = ET.SubElement(
            ruleEle, "NoncurrentVersionTransition"
        )
        if noncurrentVersionTransition.get("noncurrentDays") is not None:
            ET.SubElement(noncurrentVersionTransitionEle, "NoncurrentDays").text = (
                util.to_string(noncurrentVersionTransition["noncurrentDays"])
            )
        ET.SubElement(noncurrentVersionTransitionEle, "StorageClass").text = (
            util.to_string(
        ...
        if rules is not None and len(rules) > 0:
            for item in rules:
                ...
                if item.get("prefix") is not None:
                    ET.SubElement(ruleEle, "Prefix").text = util.safe_decode(
                        item["prefix"]
                    )
                ET.SubElement(ruleEle, "Status").text = util.to_string(
                    item.get("status")
                )
                ...
                if item.get("noncurrentVersionTransition") is not None:
                    if isinstance(item["noncurrentVersionTransition"], list):
                        for noncurrentVersionTransition in item[
                            "noncurrentVersionTransition"
                        ]:
                            self._transNoncurrentVersionTransition(
                                ruleEle, noncurrentVersionTransition
                            )
                    else:
                        self._transNoncurrentVersionTransition(
                            ruleEle, item["noncurrentVersionTransition"]
                        )

                if (
                    item.get("noncurrentVersionExpiration") is not None
                    and item["noncurrentVersionExpiration"].get("noncurrentDays")
                    is not None
                ):
                    noncurrentVersionExpirationEle = ET.SubElement(
                        ruleEle, "NoncurrentVersionExpiration"
                    )
                    ET.SubElement(
                        noncurrentVersionExpirationEle, "NoncurrentDays"
                    ).text = util.to_string(
    ...
    def _trans_lifecycle_transition_expiration(self, item, ruleEle):
        ...
        if item.get("expiration") is not None and (
            item["expiration"].get("date") is not None
            or item["expiration"].get("days") is not None
        ):
            expirationEle = ET.SubElement(ruleEle, "Expiration")
            if item["expiration"].get("days") is not None:
                ET.SubElement(expirationEle, "Days").text = util.to_string(
                    item["expiration"]["days"]
                )
            elif item["expiration"].get("date") is not None:
                date = (
                    item["expiration"]["date"].ToUTMidTime()
                    if isinstance(item["expiration"]["date"], DateTime)
                    else item["expiration"]["date"]
                )
    ...
    def trans_website(self, website):
        ...
        if website.get("redirectAllRequestTo") is not None:
            ...
            if (
                website.get("indexDocument") is not None
                and website["indexDocument"].get("suffix") is not None
            ):
                indexDocEle = ET.SubElement(root, "IndexDocument")
                ET.SubElement(indexDocEle, "Suffix").text = util.to_string(
                    website["indexDocument"]["suffix"]
                )
            if (
                website.get("errorDocument") is not None
                and website["errorDocument"].get("key") is not None
            ):
                errorDocEle = ET.SubElement(root, "ErrorDocument")
                ET.SubElement(errorDocEle, "Key").text = util.to_string(
    ...
    def _trans_website_routingRules(root, website):
        if isinstance(website.get("routingRules"), list) and bool(
            ...
            for routingRule in website["routingRules"]:
                ...
                if routingRule.get("condition") is not None:
                    ...
                    if routingRule["condition"].get("keyPrefixEquals") is not None:
                        ET.SubElement(conditionEle, "KeyPrefixEquals").text = (
                            util.to_string(routingRule["condition"]["keyPrefixEquals"])
                        )
                    if (
                        routingRule["condition"].get("httpErrorCodeReturnedEquals")
                        is not None
                    ):
                        ET.SubElement(
                            conditionEle, "HttpErrorCodeReturnedEquals"
                        ).text = util.to_string(
                ...
                if routingRule.get("redirect") is not None:
                    ...
                    if redirect.get("protocol") is not None:
                        ET.SubElement(redirectEle, "Protocol").text = util.to_string(
                            redirect["protocol"]
                        )

                    if redirect.get("hostName") is not None:
                        ET.SubElement(redirectEle, "HostName").text = util.to_string(
                            redirect["hostName"]
                        )

                    if redirect.get("replaceKeyPrefixWith") is not None:
                        ET.SubElement(redirectEle, "ReplaceKeyPrefixWith").text = (
                            util.safe_decode(redirect["replaceKeyPrefixWith"])
                        )

                    if redirect.get("replaceKeyWith") is not None:
                        ET.SubElement(redirectEle, "ReplaceKeyWith").text = (
                            util.safe_decode(redirect["replaceKeyWith"])
                        )

                    if redirect.get("httpRedirectCode") is not None:
                        ET.SubElement(redirectEle, "HttpRedirectCode").text = (
                            util.to_string(redirect["httpRedirectCode"])
                        )
        return root
    ...
    def trans_notification(self, notification):
        ...
        def _set_configuration(config_type, urn_type):
            if (
                ...
                and notification.get(config_type) is not None
                and bool(notification[config_type])
            ):
                ...
                for topicConfiguration in notification[config_type]:
                    ...
                    if topicConfiguration.get("id") is not None:
                        ET.SubElement(topicConfigurationEle, "Id").text = (
                            util.safe_decode(topicConfiguration["id"])
                        )
                    ...
                    if isinstance(topicConfiguration.get("filterRules"), list) and bool(
                        ...
                        filterRulesEle = ET.SubElement(
                            filterEle, "Object" if self.is_obs else "S3Key"
                        )
                        for filterRule in topicConfiguration["filterRules"]:
                            ...
                            if filterRule.get("name") is not None:
                                ET.SubElement(filterRuleEle, "Name").text = (
                                    util.to_string(filterRule["name"])
                                )
                            if filterRule.get("value") is not None:
                                ET.SubElement(filterRuleEle, "Value").text = (
                                    util.safe_decode(filterRule["value"])
                                )
                    _urn_type = urn_type[:1].upper() + urn_type[1:]
                    if topicConfiguration.get(urn_type) is not None:
                        ET.SubElement(topicConfigurationEle, _urn_type).text = (
                            util.to_string(topicConfiguration[urn_type])
                        )

                    if (
                        isinstance(topicConfiguration.get("events"), list)
                        and len(topicConfiguration["events"]) > 0
                    ):
                        for event in topicConfiguration["events"]:
                            ET.SubElement(topicConfigurationEle, "Event").text = (
                                util.to_string(self.ha.adapt_event_type(event))
                            )
    ...
    def trans_complete_multipart_upload_request(completeMultipartUploadRequest):
        ...
        parts = (
            []
            if completeMultipartUploadRequest.get("parts") is None
            else (
        ...
        for obj in parts:
            ...
            ET.SubElement(partEle, "PartNumber").text = util.to_string(
                obj.get("partNum")
            )
    ...
    def trans_set_bucket_acl(self, **kwargs):
        ...
        if aclControl is not None:
            self._put_key_value(
                headers, self.ha.acl_header(), self.ha.adapt_acl_control(aclControl)
            )
            entity = None
        else:
            acl = kwargs.get("acl")
    ...
    def trans_set_object_acl(self, **kwargs):
        ...
        if aclControl is not None:
            ...
            acl = kwargs.get("acl")
            entity = (
                None if acl is None or not bool(acl) else self.trans_acl(acl, False)
            )
        return {"pathArgs": pathArgs, "headers": headers, "entity": entity}
    ...
    def trans_acl(self, acl, is_bucket=True):
        ...
        if acl.get("owner") is not None:
            ...
            if owner.get("owner_name") is not None and not self.is_obs:
                ET.SubElement(ownerEle, "DisplayName").text = util.safe_decode(
                    owner["owner_name"]
                )

        if not is_bucket and self.is_obs and acl.get("delivered") is not None:
            ET.SubElement(root, "Delivered").text = util.to_string(
                acl["delivered"]
            ).lower()
        ...
        for grant in grants:
            ...
            if grant.get("grantee") is not None:
                ...
                if grantee.get("group") is not None:
                    ...
                    if group_val:
                        granteeEle = ET.SubElement(
                            grantEle, "Grantee", self._trans_grantee_is_obs(attrib)
                        )
                        ET.SubElement(
                            granteeEle, "Canned" if self.is_obs else "URI"
                        ).text = group_val
                    else:
                    ...
                    ET.SubElement(granteeEle, "ID").text = util.to_string(
                        grantee["grantee_id"]
                    )
                    if grantee.get("grantee_name") is not None and not self.is_obs:
                        ET.SubElement(granteeEle, "DisplayName").text = (
                            util.safe_decode(grantee["grantee_name"])
                        )
            if grant.get("permission") is not None:
                ET.SubElement(grantEle, "Permission").text = util.to_string(
                    grant["permission"]
                )

            if grant.get("delivered") is not None and self.is_obs:
                ET.SubElement(grantEle, "Delivered").text = util.to_string(
                    grant["delivered"]
                ).lower()
    ...
    def trans_logging(self, logging):
        ...
        if (
            logging.get("targetBucket") is not None
            or logging.get("targetPrefix") is not None
            or (
                logging.get("targetGrants") is not None
                and bool(logging["targetGrants"])
            )
        ):
            loggingEnableEle = ET.SubElement(root, "LoggingEnabled")
            if logging.get("targetBucket") is not None:
                ET.SubElement(loggingEnableEle, "TargetBucket").text = util.to_string(
                    logging["targetBucket"]
                )
            if logging.get("targetPrefix") is not None:
                ET.SubElement(loggingEnableEle, "TargetPrefix").text = util.safe_decode(
                    logging["targetPrefix"]
                )
            if (
                logging.get("targetGrants") is not None
                and len(logging["targetGrants"]) > 0
            ):
    ...
    def trans_restore(self, days, tier):
        ...
        if tier is not None:
            glacierJobEle = (
                ET.SubElement(root, "RestoreJob")
                if self.is_obs
                else ET.SubElement(root, "GlacierJobParameters")
            )
    ...
    def trans_put_object(self, **kwargs):
        ...
        if headers is not None and len(headers) > 0:
            ...
            self._put_key_value(
                _headers, self.ha.content_sha256_header(), headers.get("sha256")
            )
            self._put_key_value(
                _headers,
                self.ha.acl_header(),
                self.ha.adapt_acl_control(headers.get("acl")),
            )
            self._put_key_value(
                _headers,
                self.ha.website_redirect_location_header(),
                headers.get("location"),
            )
            self._put_key_value(
                _headers, const.CONTENT_TYPE_HEADER, headers.get("contentType")
            )
            self._set_sse_header(headers.get("sseHeader"), _headers)
            self._put_key_value(
                _headers,
                self.ha.storage_class_header(),
                self.ha.adapt_storage_class(headers.get("storageClass")),
            )
            self._put_key_value(
                _headers, const.CONTENT_LENGTH_HEADER, headers.get("contentLength")
            )
            self._put_key_value(
                _headers, self.ha.expires_header(), headers.get("expires")
            )

            if self.is_obs:
                self._put_key_value(
                    _headers,
                    self.ha.success_action_redirect_header(),
                    headers.get("successActionRedirect"),
                )

            if (
                headers.get("extensionGrants") is not None
                and len(headers["extensionGrants"]) > 0
            ):
                grantDict = {}
                for extensionGrant in headers["extensionGrants"]:
                    permission = self.ha.adapt_extension_permission(
                        extensionGrant.get("permission"), False
                    )
                    if (
                        permission is not None
                        and extensionGrant.get("granteeId") is not None
    ...
    def trans_initiate_multipart_upload(self, **kwargs):
        ...
        headers = {}
        self._put_key_value(
            headers, self.ha.acl_header(), self.ha.adapt_acl_control(kwargs.get("acl"))
        )
        self._put_key_value(
            headers,
            self.ha.storage_class_header(),
        ...
        self._put_key_value(
            headers,
            self.ha.website_redirect_location_header(),
            kwargs.get("websiteRedirectLocation"),
        )
        self._put_key_value(
            headers, const.CONTENT_TYPE_HEADER, kwargs.get("contentType")
        )
    ...
    def trans_set_object_metadata(self, **kwargs):
        ...
        if headers is not None and len(headers) > 0:
            directive = (
                "REPLACE_NEW"
                if headers.get("removeUnset") is None or not headers["removeUnset"]
                else "REPLACE"
            )
            self._put_key_value(
                _headers, self.ha.metadata_directive_header(), directive
            )
            self._put_key_value(
                _headers,
                self.ha.storage_class_header(),
            ...
            self._put_key_value(
                _headers, const.CACHE_CONTROL_HEADER, headers.get("cacheControl")
            )
            self._put_key_value(
                _headers,
                const.CONTENT_DISPOSITION_HEADER,
                headers.get("contentDisposition"),
            )
            self._put_key_value(
                _headers, const.CONTENT_ENCODING_HEADER, headers.get("contentEncoding")
            )
            self._put_key_value(
                _headers, const.CONTENT_LANGUAGE_HEADER, headers.get("contentLanguage")
            )
            self._put_key_value(
                _headers, const.CONTENT_TYPE_HEADER, headers.get("contentType")
            )
    ...
    def trans_copy_object(self, **kwargs):
        ...
        copy_source = "/%s/%s" % (
            util.to_string(kwargs.get("sourceBucketName")),
            util.to_string(kwargs.get("sourceObjectKey")),
        )
        versionId = kwargs.get("versionId")
        if versionId is not None:
        ...
        if headers is not None and len(headers) > 0:
            ...
            self._put_key_value(
                _headers, self.ha.metadata_directive_header(), headers.get("directive")
            )
            self._put_key_value(
                _headers, self.ha.copy_source_if_match_header(), headers.get("if_match")
            )
            self._put_key_value(
                _headers,
                self.ha.copy_source_if_none_match_header(),
                headers.get("if_none_match"),
            )
            self._put_key_value(
                _headers,
                self.ha.copy_source_if_modified_since_header(),
                (
                    headers["if_modified_since"].ToGMTTime()
                    if isinstance(headers.get("if_modified_since"), DateTime)
                    else headers.get("if_modified_since")
                ),
            )
            self._put_key_value(
                _headers,
                self.ha.copy_source_if_unmodified_since_header(),
                (
                    headers["if_unmodified_since"].ToGMTTime()
                    if isinstance(headers.get("if_unmodified_since"), DateTime)
                    else headers.get("if_unmodified_since")
                ),
            )
    ...
    def trans_copy_part(self, **kwargs):
        ...
        if copySourceRange is not None:
            ...
            self._put_key_value(
                headers,
                self.ha.copy_source_range_header(),
                (
                    copySourceRange
                    if copySourceRange.startswith("bytes=")
                    else "bytes=" + copySourceRange
                ),
            )
    ...
    def trans_get_object(self, **kwargs):
        ...
        if getObjectRequest is not None and len(getObjectRequest) > 0:
            self._put_key_value(
                pathArgs,
                const.RESPONSE_CACHE_CONTROL_PARAM,
                getObjectRequest.get("cache_control"),
            )
            self._put_key_value(
                pathArgs,
                const.RESPONSE_CONTENT_DISPOSITION_PARAM,
                getObjectRequest.get("content_disposition"),
            )
            self._put_key_value(
                pathArgs,
                const.RESPONSE_CONTENT_ENCODING_PARAM,
                getObjectRequest.get("content_encoding"),
            )
            self._put_key_value(
                pathArgs,
                const.RESPONSE_CONTENT_LANGUAGE_PARAM,
                getObjectRequest.get("content_language"),
            )
            self._put_key_value(
                pathArgs,
                const.RESPONSE_CONTENT_TYPE_PARAM,
                getObjectRequest.get("content_type"),
            )
            self._put_key_value(
                pathArgs, const.RESPONSE_EXPIRES_PARAM, getObjectRequest.get("expires")
            )
            self._put_key_value(
                pathArgs, const.VERSION_ID_PARAM, getObjectRequest.get("versionId")
            )
            self._put_key_value(
                pathArgs,
                const.X_IMAGE_PROCESS_PARAM,
                getObjectRequest.get("imageProcess"),
            )
        ...
        if headers is not None and len(headers) > 0:
            ...
            self._put_key_value(
                _headers, const.IF_NONE_MATCH, headers.get("if_none_match")
            )
            self._put_key_value(_headers, const.ORIGIN_HEADER, headers.get("origin"))
            self._put_key_value(
                _headers,
                const.ACCESS_CONTROL_REQUEST_HEADERS_HEADER,
    ...
    def trans_list_multipart_uploads(self, **kwargs):
        ...
        if multipart is not None:
            ...
            self._put_key_value(
                pathArgs, "upload-id-marker", multipart.get("upload_id_marker")
            )
        return {"pathArgs": pathArgs}
    ...
    def _set_source_sse_header(self, sseHeader, headers=None):
        ...
        if isinstance(sseHeader, SseCHeader):
            self._put_key_value(
                headers, self.ha.copy_source_sse_c_header(), sseHeader.get("encryption")
            )
            key = util.to_string(sseHeader.get("key"))
            self._put_key_value(
                headers, self.ha.copy_source_sse_c_key_header(), util.base64_encode(key)
            )
            self._put_key_value(
                headers,
                self.ha.copy_source_sse_c_key_md5_header(),
    ...
    def _set_sse_header(self, sseHeader, headers=None, onlySseCHeader=False):
        ...
        if isinstance(sseHeader, SseCHeader):
            self._put_key_value(
                headers, self.ha.sse_c_header(), sseHeader.get("encryption")
            )
            key = util.to_string(sseHeader.get("key"))
            self._put_key_value(
                headers, self.ha.sse_c_key_header(), util.base64_encode(key)
            )
            self._put_key_value(
                headers,
                self.ha.sse_c_key_md5_header(),
                util.base64_encode(util.md5_encode(key)),
            )
        elif isinstance(sseHeader, SseKmsHeader) and not onlySseCHeader:
            self._put_key_value(
                headers,
                self.ha.sse_kms_header(),
                (
                    sseHeader.get("encryption")
                    if self.is_obs
                    else "aws:" + util.to_string(sseHeader.get("encryption"))
                ),
            )
            if sseHeader.get("key") is not None:
                self._put_key_value(
                    headers, self.ha.sse_kms_key_header(), sseHeader["key"]
                )
        return headers
        ...
        if replication.get("replicationRules") is not None:
            for replicationRule in replication["replicationRules"]:
                ...
                if replicationRule.get("id") is not None:
                    ET.SubElement(ruleEle, "ID").text = util.safe_decode(
                        replicationRule["id"]
                    )
                ...
                if replicationRule.get("historicalObjectReplication") is not None:
                    ET.SubElement(ruleEle, "HistoricalObjectReplication").text = (
                        util.to_string(replicationRule["historicalObjectReplication"])
                    )
                ...
                if replicationRule.get("bucket") is not None:
                    ...
                    bucket_name = (
                        ...
                        if self.is_obs
                            ...
                            else "arn:aws:s3:::" + bucket_name
                        )
                    )
                    ET.SubElement(destinationEle, "Bucket").text = bucket_name

                    if replicationRule.get("storageClass") is not None:
                        ET.SubElement(destinationEle, "StorageClass").text = (
                            self.ha.adapt_storage_class(replicationRule["storageClass"])
                        )

                    if replicationRule.get("deleteData") is not None:
                        ET.SubElement(destinationEle, "DeleteData").text = (
                            util.to_string(replicationRule["deleteData"])
                        )
        return ET.tostring(root, "UTF-8")
    ...
    def trans_get_extension_headers(self, headers):
        ...
        if headers is not None and len(headers) > 0:
            self._put_key_value(
                _headers, self.ha.request_payer_header(), headers.get("requesterPayer")
            )
            self._put_key_value(
                _headers,
                self.ha.location_clustergroup_id_header(),
    ...
    def trans_set_bucket_fetch_policy(self, fetchPolicy):
        headers = {}
        self._put_key_value(
            headers, const.CONTENT_TYPE_HEADER, const.MIME_TYPES.get("json")
        )
    ...
    def parseListObjects(self, xml, headers=None):
        ...
        if contents is not None:
            for node in contents:
                ...
                if owner is not None:
                    ID = self._find_item(owner, "ID")
                    DisplayName = (
                        None if self.is_obs else self._find_item(owner, "DisplayName")
                    )
                    Owners = Owner(owner_id=ID, owner_name=DisplayName)
                isAppendable = self._find_item(node, "Type")
                key_entry = Content(
                    key=key,
                    lastModified=DateTime.UTCToLocal(lastmodified),
                    etag=etag,
                    size=util.to_long(size),
                    owner=Owners,
        ...
        return ListObjectsResponse(
            ...
            is_truncated=util.to_bool(is_truncated),
            next_marker=next_marker,
            contents=key_entries,
            commonPrefixs=common_prefixes,
    ...
    def parseGetBucketLocation(self, xml, headers=None):
        root = ET.fromstring(xml)
        location = (
            root.text if self.is_obs else self._find_item(root, "LocationConstraint")
        )
    ...
    def parseGetBucketStorageInfo(self, xml, headers=None):
        ...
        return GetBucketStorageInfoResponse(
    ...
    def parseGetBucketStoragePolicy(self, xml, headers=None):
        root = ET.fromstring(xml)
        storageClass = (
            root.text if self.is_obs else self._find_item(root, "DefaultStorageClass")
        )
    ...
    def parseGetBucketCors(self, xml, headers=None):
        ...
        if rules is not None:
            for rule in rules:
                ...
                maxAgeSecond = (
                    util.to_int(maxAgeSecond.text) if maxAgeSecond is not None else None
                )
                ...
                corsList.append(
                    CorsRule(
                        id=_id,
                        allowedMethod=allowMethod,
                        allowedOrigin=allowedOrigin,
                        allowedHeader=allowedHeader,
                        maxAgeSecond=maxAgeSecond,
    ...
    def parseListVersions(self, xml, headers=None):
        ...
        for version in versions:
            ...
            Version = ObjectVersion(
                key=Key,
                versionId=VersionId,
                isLatest=util.to_bool(IsLatest),
                lastModified=DateTime.UTCToLocal(LastModified),
                etag=ETag,
                size=util.to_long(Size),
                owner=Owners,
    ...
    def parseDeleteObjects(self, xml, headers=None):
        ...
        if deleteds:
            for d in deleteds:
                ...
                deleteMarker = (
                    util.to_bool(deleteMarker.text)
                    if deleteMarker is not None
                    else None
                )
        ...
        if errors:
            for e in errors:
                ...
                error_list.append(
                    ErrorResult(
                        key=_key, versionId=_versionId, code=_code, message=_message
                    )
                )
        return DeleteObjectsResponse(
            deleted=deleted_list, error=error_list, encoding_type=encoding_type
        )
    ...
    def parseDeleteObject(self, headers):
        ...
        deleteObjectResponse.deleteMarker = (
            util.to_bool(delete_marker) if delete_marker is not None else None
        )
    ...
    def parseGetBucketLifecycle(self, xml, headers=None):
        ...
        for rule in rules:
            ...
            noncurrentVersionExpiration = (
                NoncurrentVersionExpiration(
                    noncurrentDays=util.to_int(
                        nocurrent_expire.find("NoncurrentDays").text
                    )
                )
                if nocurrent_expire is not None
                else None
            )
            ...
            if noncurrentVersionTransis is not None:
                for noncurrentVersionTransis in noncurrentVersionTransis:
                    storageClass = self._find_item(
                        noncurrentVersionTransis, "StorageClass"
                    )
                    noncurrentDays = noncurrentVersionTransis.find("NoncurrentDays")
                    noncurrentDays = (
                        util.to_int(noncurrentDays.text)
                        if noncurrentDays is not None
                        else None
                    )
                    noncurrentVersionTransition = NoncurrentVersionTransition(
                        storageClass=storageClass, noncurrentDays=noncurrentDays
                    )
    ...
    def parseGetBucketWebsite(self, xml, headers=None):
        ...
        if routingRules is not None and len(routingRules) > 0:
            ...
            for rout in routingRules:
                ...
                KeyPrefixEquals = (
                    util.to_string(KeyPrefixEquals.text)
                    if KeyPrefixEquals is not None
                    else None
                )
                ...
                HttpErrorCodeReturnedEquals = (
                    util.to_int(HttpErrorCodeReturnedEquals.text)
                    if HttpErrorCodeReturnedEquals is not None
                    else None
                )
                ...
                ReplaceKeyPrefixWith = self._find_item(
                ...
                HttpRedirectCode = (
                    util.to_int(HttpRedirectCode.text)
                    if HttpRedirectCode is not None
                    else None
                )
                redirect = Redirect(
                    protocol=Protocol,
    ...
    def parseGetBucketNotification(self, xml, headers=None):
            ...
            if topicConfigurations is not None:
                ...
                for topicConfiguration in topicConfigurations:
                    ...
                    filterRules = topicConfiguration.findall(
                        "Filter/Object/FilterRule"
                        if self.is_obs
                        else "Filter/S3Key/FilterRule"
                    )
                    if filterRules is not None:
                        for filterRule in filterRules:
                            ...
                            fr = FilterRule(
                                name=(
                                    util.to_string(name.text)
                                    if name is not None
                                    else None
                                ),
                                value=(
                                    util.to_string(value.text)
                                    if value is not None
                                    else None
                                ),
                            )
        ...
        notification.topicConfigurations = _get_configuration(
            TopicConfiguration, "TopicConfiguration", "Topic"
        )
        notification.functionGraphConfigurations = _get_configuration(
            FunctionGraphConfiguration, "FunctionGraphConfiguration", "FunctionGraph"
        )
    ...
    def parseListMultipartUploads(self, xml, headers=None):
        ...
        IsTruncated = (
            util.to_bool(IsTruncated.text) if IsTruncated is not None else None
        )
        ...
        if rules:
            for rule in rules:
                ...
                DisplayName = (
                    None
                    if self.is_obs
                    else self._find_item(rule, "Initiator/DisplayName")
                )
                initiator = Initiator(id=ID, name=DisplayName)

                owner_id = self._find_item(rule, "Owner/ID")
                owner_name = (
                    None if self.is_obs else self._find_item(rule, "Owner/DisplayName")
                )
                ...
                Initiated = (
                    DateTime.UTCToLocal(Initiated.text)
    ...
    def parseCompleteMultipartUpload(self, xml, headers=None):
        ...
        completeMultipartUploadResponse.versionId = headers.get(
            self.ha.version_id_header()
        )
        completeMultipartUploadResponse.sseKms = headers.get(self.ha.sse_kms_header())
        completeMultipartUploadResponse.sseKmsKey = headers.get(
            self.ha.sse_kms_key_header()
        )
        completeMultipartUploadResponse.sseC = headers.get(self.ha.sse_c_header())
        completeMultipartUploadResponse.sseCKeyMd5 = headers.get(
            self.ha.sse_c_key_md5_header().lower()
        )
    ...
    def parseListParts(self, xml, headers=None):
        ...
        partNumbermarker = (
            util.to_int(partNumbermarker.text) if partNumbermarker is not None else None
        )
        nextPartNumberMarker = root.find("NextPartNumberMarker")
        nextPartNumberMarker = (
            util.to_int(nextPartNumberMarker.text)
            if nextPartNumberMarker is not None
            else None
        )
        ...
        displayname = (
            None if self.is_obs else self._find_item(root, "Initiator/DisplayName")
        )
    ...
    def _parseListPartsHandleParts(self, root):
        ...
        if part_list:
            for part in part_list:
                ...
                part_number = (
                    util.to_int(part_number.text) if part_number is not None else None
                )
                modified_date = part.find("LastModified")
                modified_date = (
                    DateTime.UTCToLocal(modified_date.text)
    ...
    def parseGrants(self, grants, headers=None):
        ...
        if grants is not None:
            if self.is_obs:
                ...
                for grant in grants:
                    if (
                        grant.find("Grantee").attrib.get("{0}type".format(ns))
                        == "Group"
                    ):
                        ...
                        owner_name = (
                            None
                            if self.is_obs
                            else self._find_item(grant, "Grantee/DisplayName")
                        )
    ...
    def _parseGrantsIsObs(self, grants, grant_list):
        for grant in grants:
            ...
            delivered = (
                util.to_string(delivered.text) if delivered is not None else None
            )
            cur_grant = Grant(
                grantee=grantee, permission=permission, delivered=delivered == "true"
            )
            grant_list.append(cur_grant)
        return grant_list
    ...
    def parseGetBucketLogging(self, xml, headers=None):
        ...
        return Logging(
            targetBucket=bucket,
            targetPrefix=prefix,
            targetGrants=self.parseGrants(grants),
            agency=agency,
        )
    ...
    def parseGetObjectAcl(self, xml, headers=None):
        ...
        return ACL(
            owner=owner,
            grants=self.parseGrants(grants),
    ...
    def parseCopyObject(self, xml, headers=None):
        ...
        lastModified = (
            DateTime.UTCToLocal(lastModified.text) if lastModified is not None else None
        )
        eTag = self._find_item(root, "ETag")
        ...
        copyObjectResponse.copySourceVersionId = headers.get(
            self.ha.copy_source_version_id()
        )
        ...
        copyObjectResponse.sseCKeyMd5 = headers.get(
            self.ha.sse_c_key_md5_header().lower()
        )
    ...
    def _parseGetObjectCommonHeader(self, headers, option):
        ...
        option.versionId = headers.get(self.ha.version_id_header())
        option.websiteRedirectLocation = headers.get(
            self.ha.website_redirect_location_header()
        )
        option.sseKms = headers.get(self.ha.sse_kms_header())
        option.sseKmsKey = headers.get(self.ha.sse_kms_key_header())
        ...
        option.etag = headers.get(const.ETAG_HEADER.lower())
        option.contentLength = util.to_long(
            headers.get(const.CONTENT_LENGTH_HEADER.lower())
        )
    ...
    def parseGetObjectMetadata(self, headers):
        ...
        if option.isAppendable:
            option.nextPosition = util.to_long(
                headers.get(self.ha.next_position_header())
            )
        return option
    ...
    def parseGetObject(self, headers, option):
        ...
        option.contentDisposition = headers.get(
            const.CONTENT_DISPOSITION_HEADER.lower()
        )
    ...
    def parseUploadPart(self, headers):
        ...
        uploadPartResponse.sseCKeyMd5 = headers.get(
            self.ha.sse_c_key_md5_header().lower()
        )
    ...
    def parseCopyPart(self, xml, headers=None):
        ...
        copyPartResponse = CopyPartResponse(
            modifiedDate=lastModified, lastModified=lastModified, etag=etag
        )
        ...
        copyPartResponse.sseCKeyMd5 = headers.get(
            self.ha.sse_c_key_md5_header().lower()
        )
    ...
    def parseGetBucketReplication(self, xml, headers=None):
        ...
        if rules is not None:
            for rule in rules:
                ...
                historicalObjectReplication = self._find_item(
                    rule, "Destination/HistoricalObjectReplication"
                )
                _rules.append(
                    ReplicationRule(
                        id=_id,
    ...
    def parseGetBucketFetchJob(self, jsons, header=None):
        ...
        job = result.get("job")
        if job is None:
            response = GetBucketFetchJobResponse(
                code=code, status=status, job=None, err=err
            )
            return response
        ...
        response = GetBucketFetchJobResponse(
            code=code, status=status, job=fetchJobResponse, err=err
        )
        return response
    ...
    def parseGetWorkflowTemplateResponse(jsons, header=None):
        ...
        return GetWorkflowTemplateResponse(
    ...
    def parseCreateWorkflowResponse(jsons, header=None):
        ...
        return CreateWorkflowResponse(
            graphName=graphName, graphUrn=graphUrn, createdAt=createdAt
        )
    ...
    def parseUpdateWorkflowResponse(jsons, header=None):
        ...
        return UpdateWorkflowResponse(
            graphName=graphName, graphUrn=graphUrn, lastModified=lastModified
        )
    ...
    def parseListWorkflowResponse(jsons, header=None):
        ...
        return ListWorkflowResponse(
            graphs=graphs, count=count, nextStart=nextStart, isTruncated=isTruncated
        )
    ...
    def parseAsyncAPIStartWorkflowResponse(jsons, header=None):
        ...
        return AsyncAPIStartWorkflowResponse(
            executionUrn=executionUrn, startedAt=startedAt, executionName=executionName
        )
    ...
    def parseListWorkflowExecutionResponse(jsons, header=None):
        ...
        return ListWorkflowExecutionResponse(
            count=count,
    ...
    def parseRestoreFailedWorkflowExecutionResponse(jsons, header=None):
        ...
        executionName = result.get("execution_name")
        return RestoreFailedWorkflowExecutionResponse(
            executionUrn=executionUrn,
    ...
    def trans_set_bucket_alias(self, **kwargs):
        ...
        entity = (
            None
            if aliasInfo is None or len(aliasInfo) == 0
            else self.trans_set_aliasInfo(aliasInfo)
        )
    ...
    def trans_set_aliasInfo(self, aliasInfo):
        ...
        ET.SubElement(bucketListEle, "Bucket").text = util.to_string(
            aliasInfo.get("bucket1")
        )
        ET.SubElement(bucketListEle, "Bucket").text = util.to_string(
            aliasInfo.get("bucket2")
        )
        return ET.tostring(root, "UTF-8")
    ...
    def trans_bind_bucket_alias(self, **kwargs):
        ...
        entity = (
            None
            if aliasInfo is None or len(aliasInfo) == 0
            else self.trans_bind_aliasInfo(aliasInfo)
        )
