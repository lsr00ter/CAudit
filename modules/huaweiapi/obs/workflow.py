...
def entrance(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if workflowClient:
                workflowClient.log_client.log(
                    INFO,
                    "%s cost %s ms"
                    % (func.__name__, int((time.time() - start) * 1000)),
                )
...
class WorkflowClient(_BasicClient):
    ...
    def __init__(self, isJsonResult=False, *args, **kwargs):
        if kwargs.get("path_style") or kwargs.get("is_signature_negotiation"):
            raise Exception(
                "Path style or signature negotiation does not support custom settings for workflow client."
            )
    ...
    def createWorkflowTemplate(
        self, templateName, description=None, states=None, inputs=None, tags=None
    ):
        return self._make_post_request(
            ...
            methodName=(
                ...
                if not self.__isJsonResult
                else self.__defaultMethodName
            ),
        )
    ...
    @entrance
    def createWorkflow(
        self, templateName, graphName, agency, description=None, parameters=None
    ):
        return self._make_post_request(
    ...
    def listWorkflow(self, graphNamePrefix=None, start=None, limit=None):
        ...
        return self._make_get_request(
            ...
            objectKey=self.__resource
            + combine(
                const.WORKFLOWS, graphNamePrefix if graphNamePrefix is not None else ""
            ),
            pathArgs=pathArgs,
            headers=prepareHeader(),
            methodName=(
    ...
    def listWorkflowExecution(
        ...
        pathArgs = _listWorkflowExecutionPathArgs(
            graphName, nextMarker, limit, executionType
        )
        if executionState:
            ...
            resp = self._make_get_request(
                ...
                pathArgs=pathArgs,
                headers=prepareHeader(),
                methodName=_listWorkflowExecutionMethodName(
                    self.__isJsonResult, self.__defaultMethodName
                ),
            )
            if resp.status > 300:
                return resp

            body = _listWorkflowExecutionBody(
                self.__isJsonResult, const.IS_PYTHON2, resp.body
            )
            ...
            nextMarker = _listWorkflowExecutionNextMarker(
                isTruncated, self.__isJsonResult, body
            )
            ...
            if len(result) == limit:
                ...
                while len(result) < limit and isTruncated:
                    ...
                    body = _listWorkflowExecutionBody(
                        self.__isJsonResult, const.IS_PYTHON2, tempResp.body
                    )
                    ...
                    isTruncated = _listWorkflowExecutionIsTruncated(
                        self.__isJsonResult, body
                    )
                    nextMarker = _listWorkflowExecutionNextMarker(
                        isTruncated, self.__isJsonResult, body
                    )

            return _listWorkflowExecutionResp(
                self.__isJsonResult, resp, result, isTruncated, nextMarker
            )
        else:
            return self._make_get_request(
                bucketName=None,
    ...
    def getWorkflowExecution(self, executionName, graphName):
        ...
        return self._make_get_request(
            bucketName=None,
            objectKey=self.__resource
            + combine(const.WORKFLOW_EXECUTIONS, executionName),
            pathArgs=pathArgs,
            headers=prepareHeader(),
            methodName=(
    ...
    def putTriggerPolicy(self, bucketName, rules):
        pathArgs = {const.WORKFLOW_TRIGGERPOLICY: None}
