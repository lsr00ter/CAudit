...
from modules.huaweiapi.obs import (
    auth,
    const,
    convertor,
    loadtoken,
...
from modules.huaweiapi.obs.ilog import (
    DEBUG,
    ERROR,
    INFO,
    LogClient,
...
from modules.huaweiapi.obs.model import (
    ACL,
    AppendObjectContent,
    AppendObjectHeader,
    BaseModel,
...
class _SecurityProvider(object):
    def __init__(self, access_key_id, secret_access_key, security_token=None):
        ...
        security_token = (
            util.to_string(util.safe_encode(security_token)).strip()
...
def _wrapperFinally(obsClient, name, start):
    if obsClient:
        obsClient.log_client.log(
            INFO, "%s cost %s ms" % (name, int((time.time() - start) * 1000))
        )
        if obsClient.is_signature_negotiation and hasattr(
            obsClient.thread_local, "signature"
        ):
...
def _isCreateBucket(obsClient, name, key):
    return (
        obsClient._getApiVersion()
        if name == "createBucket"
        else obsClient._getApiVersion(key)
    )
        ...
        if not result_dic:
            with locks.get_lock(hash(key) % locks.LOCK_COUNT):
                ...
                if not result_dic:
                    ...
                    result_dic = {
                        "signature": authType,
                        "expire": random.randint(900, 1200) + caches.nowTime(),
                    }
                    if name != "createBucket":
            ...
            if obsClient:
                ...
                if obsClient.is_signature_negotiation:
                    ...
                    if func.__name__ in black_list:
                        ...
                        authType, resp = _is_signature_negotiation(
                            obsClient, func.__name__, key
                        )
                        if not authType:
                            return resp
...
class HaWrapper(object):
    ...
    def __getattr__(self, item):
        signature = (
            self.thread_local.signature
            if hasattr(self.thread_local, "signature")
            else self.signature
        )
        ha = (
            self.obsHA
            if signature == "obs"
            else self.v4HA if signature == "v4" else self.v2HA
        )
...
class ConvertWrapper(object):
    ...
    def __getattr__(self, item):
        ...
        convert = (
            self.obsCover
            if signature == "obs"
            else self.v4Cover if signature == "v4" else self.v2Cover
        )
...
class _BasicClient(object):
    def __init__(
        ...
        security_provider_policy=None,
        client_mode="obs",
    ):
        self.securityProvider = _SecurityProvider(
    ...
    def _get_token(self):
        ...
        try:
            if self.security_provider_policy is not None:
                if (
                    self.securityProvider.access_key_id != ""
                    and self.securityProvider.secret_access_key != ""
                ):
                    return self.securityProvider

                value_dict = get_token(
                    self.security_providers, name=self.security_provider_policy
                )
                securityProvider = _SecurityProvider(
                    value_dict.get("accessKey"),
                    value_dict.get("secretKey"),
        ...
        try:
            ...
            if hasattr(ssl, "SSLContext"):
                ...
                if custom_ciphers is not None:
                    ...
                    if (
                        custom_ciphers != ""
                        and hasattr(context, "set_ciphers")
                        and callable(context.set_ciphers)
                    ):
    ...
    def initLog(self, log_config=None, log_name="OBS_LOGGER"):
        if log_config:
            self.log_client = LogClient(
                log_config, "OBS_LOGGER" if const.IS_WINDOWS else log_name, log_name
            )
            ...
            msg.append(
                ...
                + (
                    "%s://%s:%d"
                    % ("https" if self.is_secure else "http", self.server, self.port)
                )
            )
            msg.append(
                "Access Mode="
                + ("Path" if self.path_style else "Virtual Hosting")
                + "]"
            )
    ...
    def _generate_object_url(self, ret, bucketName, objectKey):
        if ret and ret.status < 300 and ret.body:
            ret.body.objectUrl = self.calling_format.get_full_url(
                self.is_secure, self.server, self.port, bucketName, objectKey, {}
            )

    def _make_options_request(
        self,
        bucketName,
        objectKey=None,
        pathArgs=None,
    ...
    def _make_request_with_retry(
        ...
        while True:
            try:
                ...
                return (
                    self._parse_xml(conn, methodName, readable)
                    if not parseMethod
                    else parseMethod(conn)
                )
            except Exception as e:
                ret = None
                ...
                if isinstance(e, _InternalException):
                    ...
                    if isinstance(e, _RedirectException):
                        ...
                        ret = e.result
                        if (
                            methodType == const.HTTP_METHOD_GET
                            and e.result.status == 302
                        ):
                            redirectFlag = True
                if redirect_count >= self.max_redirect_count:
                    self.log_client.log(
                        ERROR,
                        "request redirect count [%d] greater than max redirect count [%d]"
    ...
    def _make_request_internal(
        ...
        if redirectLocation:
            ...
            connect_server = (
                self.server
                if self.is_cname
                else self.calling_format.get_server(self.server, bucketName)
            )
            redirect = False
            port = self.port
        ...
        headers[const.HOST_HEADER] = (
            "%s:%s" % (connect_server, port)
            if port not in (443, 80)
            else connect_server
        )
        header_config = self._add_auth_headers(
            headers, method, bucketName, objectKey, pathArgs, skipAuthentication
        )
        ...
        self.log_client.log(
            DEBUG, "method:%s, path:%s, header:%s", method, path, header_log
        )
        conn = self._send_request(
            connect_server,
            method,
            path,
            header_config,
    ...
    def _parse_entity(entity, headers):
        if entity is not None and not callable(entity):
            ...
            if not const.IS_PYTHON2:
                entity = (
                    entity.encode("UTF-8") if not isinstance(entity, bytes) else entity
                )
            headers[const.CONTENT_LENGTH_HEADER] = util.to_string(len(entity))
        return entity, headers

    def _add_auth_headers(
        self, headers, method, bucketName, objectKey, pathArgs, skipAuthentication=False
    ):
        ...
        if util.is_valid(ak) and util.is_valid(sk):
            if securityProvider.security_token is not None:
                headers[self.ha.security_token_header()] = (
                    securityProvider.security_token
                )

            cur_signature = (
                self.thread_local.signature.lower()
                if self.is_signature_negotiation
                else self.signature.lower()
            )
            if cur_signature == "v4":
                if now_date is None:
                    now_date = datetime.strptime(
                        headers[self.ha.date_header()], const.LONG_DATE_FORMAT
                    )
                shortDate = now_date.strftime(const.SHORT_DATE_FORMAT)
                longDate = now_date.strftime(const.LONG_DATE_FORMAT)
                v4Auth = auth.V4Authentication(
                    ak,
                    sk,
                    str(self.region) if self.region is not None else "",
                    shortDate,
                    longDate,
                    self.path_style,
                    self.ha,
                ...
                if self.ha.security_token_header() in headers:
                    log_canonical_request = str.replace(
                        log_canonical_request,
                        headers[self.ha.security_token_header()],
                        "******",
                    )
                self.log_client.log(
                    DEBUG, "%s: %s" % (const.CANONICAL_REQUEST, log_canonical_request)
                )
            else:
                obsAuth = auth.Authentication(
                    ak, sk, self.path_style, self.ha, self.server, self.is_cname
                )
                ret = obsAuth.doAuth(method, bucketName, objectKey, pathArgs, headers)
                log_canonical_string = ret[const.CANONICAL_STRING]
                if self.ha.security_token_header() in headers:
                    log_canonical_string = str.replace(
                        log_canonical_string,
                        headers[self.ha.security_token_header()],
                        "******",
                    )
                self.log_client.log(
                    DEBUG, "%s: %s" % (const.CANONICAL_STRING, log_canonical_string)
                )
        ...
        if isinstance(headers, dict):
            for k, v in headers.items():
                if k is not None and v is not None:
                    ...
                    if (
                        k.lower()
                        not in const.ALLOWED_REQUEST_HTTP_HEADER_METADATA_NAMES
                        and not k.lower().startswith(const.V2_HEADER_PREFIX)
                        and not k.lower().startswith(const.OBS_HEADER_PREFIX)
                    ):
                        if method not in (
                            const.HTTP_METHOD_PUT,
                            const.HTTP_METHOD_POST,
    ...
    def _rename_request_headers_handle(self, k, v, new_headers):
        if k.lower() == self.ha._get_header_prefix() + "copy-source":
            ...
            new_headers[k] = (
                v if (isinstance(v, list)) else util.encode_item(v, " ;/?:@&=+$,'*")
            )
        return new_headers
    ...
    def _get_server_connection(
        ...
        is_secure = (
            self.is_secure if scheme is None else True if scheme == "https" else False
        )

        if (
            self.connHolder is not None
            and not self.connHolder["connSet"].empty()
            and not redirect
        ):
            try:
        ...
        if self.use_http2:
            ...
            conn = http2._get_server_connection(
                server, port, self.context, is_secure, proxy_host, proxy_port
            )
        else:
            conn = self._get_server_connection_use_http1x(
                is_secure, server, port, proxy_host, proxy_port
            )
    ...
    def _get_server_connection_use_http1x(
        self, is_secure, server, port, proxy_host, proxy_port
    ):
            ...
            if const.IS_PYTHON2:
                try:
                    conn = httplib.HTTPSConnection(
                        server, port=port, timeout=self.timeout, context=self.context
                    )
                except Exception:
                    conn = httplib.HTTPSConnection(
                        server, port=port, timeout=self.timeout
                    )
            else:
                conn = httplib.HTTPSConnection(
                    server,
    ...
    def _send_request(
        ...
        conn, header = self._parse_request_connection(
            server, port, scheme, connection_key, redirect, header
        )
        ...
        if method == const.HTTP_METHOD_OPTIONS and not self.use_http2:
            ...
            if self.use_http2:
                ...
                self._parse_connection_chunked_mode(
                    conn, chunkedMode, method, path, header
                )
                if entity is not None:
                    if callable(entity):
                        ...
                        conn.send(entity)
                        self.log_client.log(
                            DEBUG, "request content:%s", util.to_string(entity)
                        )
        return conn

    def _parse_request_connection(
        self, server, port, scheme, connection_key, redirect, header
    ):
        if self.proxy_host is not None and self.proxy_port is not None:
            conn = self._get_server_connection(
                ...
                scheme,
                redirect,
                util.to_string(self.proxy_host),
                util.to_int(self.proxy_port),
            )
            _header = {}
            if self.proxy_username is not None and self.proxy_password is not None:
                _header[const.PROXY_AUTHORIZATION_HEADER] = "Basic " "%s" % (
                    util.base64_encode(
                        util.to_string(self.proxy_username)
                        + ":"
                        + util.to_string(self.proxy_password)
                    )
                )
            if not self.use_http2:
                conn.set_tunnel(server, port, _header)
            else:
                header[const.PROXY_AUTHORIZATION_HEADER] = _header[
                    const.PROXY_AUTHORIZATION_HEADER
                ]
    ...
    def _parse_content(
        ...
        download_start="",
        downloadPath=None,
        chuckSize=const.READ_ONCE_LENGTH,
        ...
        try:
            ...
            content_length = (
                util.to_long(content_length) if content_length is not None else None
            )
            if not notifier:
                notifier = self._get_notifier(content_length, progressCallback)
                notifier.start()
            result_wrapper = ResponseWrapper(
                conn, response, self.connHolder, content_length, notifier
            )
            if loadStreamInMemory:
                self.log_client.log(
                    DEBUG, "loadStreamInMemory is True, read stream into memory"
                )
                buf = self._get_buffer_data(result_wrapper, chuckSize)
                body = ObjectStream(
                    buffer=buf, size=util.to_long(len(buf)) if buf is not None else 0
                )
                ...
                self.log_client.log(
                    DEBUG, "DownloadPath is " + util.to_string(file_path)
                )
    ...
    def _get_notifier(content_length, progressCallback):
        return (
            progress.ProgressNotifier(progressCallback, content_length)
            if content_length is not None
            and content_length > 0
            and progressCallback is not None
    ...
    def _rename_key(k, v):
        ...
        if k.startswith(const.V2_META_HEADER_PREFIX):
            k = k[
                k.index(const.V2_META_HEADER_PREFIX)
                + len(const.V2_META_HEADER_PREFIX) :
            ]
            ...
            k = k[
                k.index(const.OBS_META_HEADER_PREFIX)
                + len(const.OBS_META_HEADER_PREFIX) :
            ]
            k = util.decode_item(k)
            v = util.decode_item(v)
            flag = 1
    ...
    def _prepare_body(self, methodName, responseData, isJson, headers):
        ...
        if methodName is not None:
            parseMethod = getattr(
                self.convertor, "parse" + methodName[:1].upper() + methodName[1:]
            )
                ...
                try:
                    if responseData:
                        responseData = (
                            responseData
                            if const.IS_PYTHON2
                            else responseData.decode("UTF-8")
                        )
                        ...
                        if not isJson:
                            ...
                            responseData = (
                                responseData
                                if search is None
                                else responseData.replace(search.group(), "")
                            )
    ...
    def _is_redirect_exception(status, headers):
        return (
            300 <= status < 400
            and status != 304
            and const.LOCATION_HEADER.lower() in headers
        )

    def _parse_xml_internal(
        self, result, methodName=None, chuckSize=const.READ_ONCE_LENGTH, readable=False
    ):
        ...
        header = self._rename_response_headers(headers)
        isJson = headers.get(
            const.CONTENT_TYPE_HEADER.lower(), "xml"
        ) == const.MIME_TYPES.get("json")
        indicator = headers.get(self.ha.indicator_header())

        if status < 300:
            responseData, body = self._prepare_body(
                methodName, responseData, isJson, headers
            )
            requestId = self._prepare_request_id(None, headers)

        elif responseData:
            responseData = (
                responseData if const.IS_PYTHON2 else responseData.decode("UTF-8")
            )
            try:
                if isJson:
                    code, message, requestId = self.convertor.parseJsonErrorResult(
                        responseData
                    )
                else:
                    search = self.pattern.search(responseData)
                    ...
                    code, message, requestId, hostId, resource = (
                        self.convertor.parseErrorResult(responseData)
                    )
                body = responseData
        ...
        if status >= 300:
            self.log_client.log(
                ERROR,
                "exceptional obs response:status:%d,reason:%s,code:%s,message:%s,requestId:%s",
                status,
                reason,
                code,
                message,
        ...
        if not readable:
            if self._is_redirect_exception(status, headers):
                ...
                self.log_client.log(
                    WARNING, "http code is %d, need to redirect to %s", status, location
                )
...
class _CreatePostSignatureResponse(BaseModel):
    allowedAttr = {
        "originPolicy": const.BASESTRING,
        "policy": const.BASESTRING,
        "credential": const.BASESTRING,
        "date": const.BASESTRING,
        "signature": const.BASESTRING,
...
class ObsClient(_BasicClient):
    ...
    def _prepareParameterForSignedUrl(
        self, specialParam, expires, headers, queryParams
    ):

        headers = (
            {} if headers is None or not isinstance(headers, dict) else headers.copy()
        )
        queryParams = (
            {}
            if queryParams is None or not isinstance(queryParams, dict)
            else queryParams.copy()
        )
        ...
        _queryParams = self._prepareParameterForSignedUrlQuery(
    ...
    def _prepareParameterForSignedUrlQuery(self, queryParams, specialParam):
        ...
        if specialParam is not None:
            specialParam = (
                "storageClass"
                if self.signature.lower() == "obs" and specialParam == "storagePolicy"
                else (
                    "storagePolicy"
                    if self.signature.lower() != "obs"
                    and specialParam == "storageClass"
                    else specialParam
                )
            )
    ...
    def createSignedUrl(
        ...
        delegate = (
            self._createV4SignedUrl
            if self.signature.lower() == "v4"
            else self._createV2SignedUrl
        )
        return delegate(
            method, bucketName, objectKey, specialParam, expires, headers, queryParams
        )

    def createV2SignedUrl(
        self,
        method,
        bucketName=None,
        objectKey=None,
        ...
        return self._createV2SignedUrl(
            method, bucketName, objectKey, specialParam, expires, headers, queryParams
        )

    def createV4SignedUrl(
        self,
        method,
        bucketName=None,
        objectKey=None,
        ...
        return self._createV4SignedUrl(
            method, bucketName, objectKey, specialParam, expires, headers, queryParams
        )

    def _createV2SignedUrl(
        self,
        method,
        bucketName=None,
        objectKey=None,
        ...
        headers, queryParams, expires, calling_format = (
            self._prepareParameterForSignedUrl(
                specialParam, expires, headers, queryParams
            )
        )
        ...
        connect_server = (
            self.server
            if self.is_cname
            else calling_format.get_server(self.server, bucketName)
        )

        headers[const.HOST_HEADER] = (
            "%s:%s" % (connect_server, self.port)
        ...
        securityProvider = self._get_token()
        if (
            securityProvider.security_token is not None
            and self.ha.security_token_header() not in queryParams
        ):
            queryParams[self.ha.security_token_header()] = (
                securityProvider.security_token
            )

        v2Auth = auth.Authentication(
            securityProvider.access_key_id,
            securityProvider.secret_access_key,
            self.path_style,
            self.ha,
            self.server,
            self.is_cname,
        ...
        signature = v2Auth.getSignature(
            method, bucketName, objectKey, queryParams, headers, util.to_string(expires)
        )["Signature"]

        queryParams["Expires"] = expires
        queryParams["AccessKeyId" if self.signature == "obs" else "AWSAccessKeyId"] = (
            securityProvider.access_key_id
        )
        ...
        result = {
            "signedUrl": calling_format.get_full_url(
                self.is_secure,
                self.server,
                self.port,
                bucketName,
    ...
    def _createV4SignedUrl(
        ...
        date = (
            headers[const.DATE_HEADER]
            if const.DATE_HEADER in headers
            else headers.get(const.DATE_HEADER.lower())
        )
        date = (
            datetime.strptime(date, const.GMT_DATE_FORMAT)
            if date
    ...
    def createV4PostSignature(
        self, bucketName=None, objectKey=None, expires=300, formParams=None
    ):
        return self._createPostSignature(
            bucketName, objectKey, expires, formParams, True
        )

    def createPostSignature(
        self, bucketName=None, objectKey=None, expires=300, formParams=None
    ):
        return self._createPostSignature(
            bucketName, objectKey, expires, formParams, self.signature.lower() == "v4"
        )

    def _createPostSignature(
        self, bucketName=None, objectKey=None, expires=300, formParams=None, is_v4=False
    ):
        ...
        formParams = self._parse_post_params(
            formParams,
            securityProvider,
            is_v4,
            bucketName,
        ...
        for key, value in formParams.items():
            if key:
                ...
                if (
                    key not in const.ALLOWED_REQUEST_HTTP_HEADER_METADATA_NAMES
                    and not key.startswith(self.ha._get_header_prefix())
                    and not key.startswith(const.OBS_HEADER_PREFIX)
                    and key not in conditionAllowKeys
                ):
                    continue
        ...
        result = self._parse_post_signature_type(
            is_v4,
            securityProvider,
            originPolicy,
            policy,
    ...
    def _parse_post_params(
        ...
        formParams = (
            ...
            else formParams.copy()
        )

        if (
            securityProvider.security_token is not None
            and self.ha.security_token_header() not in formParams
        ):
            formParams[self.ha.security_token_header()] = (
                securityProvider.security_token
            )
    ...
    def _parse_post_signature_type(
        self,
        is_v4,
        securityProvider,
        originPolicy,
        ...
        if is_v4:
            ...
            signingKey = (
                v4Auth.getSigningKey_python2()
                if const.IS_PYTHON2
                else v4Auth.getSigningKey_python3()
            )
            signature = v4Auth.hmacSha256(
                signingKey, policy if const.IS_PYTHON2 else policy.encode("UTF-8")
            )
            result = {
    ...
    def _getApiVersion(self, bucketName=""):
        res = self._make_head_request(
            bucketName, pathArgs={"apiversion": None}, skipAuthentication=True
        )
    ...
    def listBuckets(self, isQueryLocation=True, extensionHeaders=None, bucketType=None):
        ...
        return self._make_get_request(
            ...
            extensionHeaders=extensionHeaders,
            **self.convertor.trans_list_buckets(
                isQueryLocation=isQueryLocation, bucketType=bucketType
            )
        )

    @funcCache
    def createBucket(
        self, bucketName, header=None, location=None, extensionHeaders=None
    ):
        if header is None:
        ...
        try:
            if (
                self.is_signature_negotiation
                and res.status == 400
                and res.errorMessage == "Unsupported Authorization Type"
                and self.thread_local.signature == const.OBS_SIGNATURE
            ):
                self.thread_local.signature = const.V2_SIGNATURE
                res = self._make_put_request(
                    bucketName,
                    extensionHeaders=extensionHeaders,
                    **self.convertor.trans_create_bucket(
    ...
    def listObjects(
        ...
        return self._make_get_request(
            ...
            extensionHeaders=extensionHeaders,
            **self.convertor.trans_list_objects(
                prefix=prefix,
                marker=marker,
                max_keys=max_keys,
                delimiter=delimiter,
    ...
    def headObject(self, bucketName, objectKey, versionId=None, extensionHeaders=None):
        ...
        return self._make_head_request(
            bucketName, objectKey, pathArgs=pathArgs, extensionHeaders=extensionHeaders
        )

    @funcCache
    def getBucketMetadata(
        self, bucketName, origin=None, requestHeaders=None, extensionHeaders=None
    ):
        return self._make_head_request(
            bucketName,
            methodName="getBucketMetadata",
            extensionHeaders=extensionHeaders,
            **self.convertor.trans_get_bucket_metadata(
                origin=origin, requestHeaders=requestHeaders
            )
        )
    ...
    def setBucketQuota(self, bucketName, quota, extensionHeaders=None):
        ...
        return self._make_put_request(
            bucketName,
            pathArgs={"quota": None},
            entity=self.convertor.trans_quota(quota),
            extensionHeaders=extensionHeaders,
        )
    ...
    def getBucketStorageInfo(self, bucketName, extensionHeaders=None):
        return self._make_get_request(
            bucketName,
            pathArgs={"storageinfo": None},
            methodName="getBucketStorageInfo",
            extensionHeaders=extensionHeaders,
        )

    @funcCache
    def setBucketAcl(
        self, bucketName, acl=None, aclControl=None, extensionHeaders=None
    ):
        if acl is None:
            acl = ACL()
    ...
    def deleteBucketPolicy(self, bucketName, extensionHeaders=None):
        return self._make_delete_request(
            bucketName, pathArgs={"policy": None}, extensionHeaders=extensionHeaders
        )
    ...
    def listMultipartUploads(
        self, bucketName, multipart=None, extensionHeaders=None, encoding_type=None
    ):
        ...
        return self._make_get_request(
            ...
            extensionHeaders=extensionHeaders,
            **self.convertor.trans_list_multipart_uploads(
                multipart=multipart, encoding_type=encoding_type
            )
        )
    ...
    def deleteBucketLifecycle(self, bucketName, extensionHeaders=None):
        return self._make_delete_request(
            bucketName, pathArgs={"lifecycle": None}, extensionHeaders=extensionHeaders
        )
    ...
    def deleteBucketWebsite(self, bucketName, extensionHeaders=None):
        return self._make_delete_request(
            bucketName, pathArgs={"website": None}, extensionHeaders=extensionHeaders
        )
    ...
    def setBucketWebsite(self, bucketName, website, extensionHeaders=None):
        ...
        return self._make_put_request(
            bucketName,
            pathArgs={"website": None},
            entity=self.convertor.trans_website(website),
            extensionHeaders=extensionHeaders,
        )
    ...
    def setBucketLogging(self, bucketName, logstatus=None, extensionHeaders=None):
        ...
        return self._make_put_request(
            bucketName,
            pathArgs={"logging": None},
            entity=self.convertor.trans_logging(logstatus),
            extensionHeaders=extensionHeaders,
        )
    ...
    def deleteBucketTagging(self, bucketName, extensionHeaders=None):
        return self._make_delete_request(
            bucketName, pathArgs={"tagging": None}, extensionHeaders=extensionHeaders
        )
    ...
    def deleteBucketCors(self, bucketName, extensionHeaders=None):
        return self._make_delete_request(
            bucketName, pathArgs={"cors": None}, extensionHeaders=extensionHeaders
        )
    ...
    def optionsBucket(self, bucketName, option, extensionHeaders=None):
        return self.optionsObject(
            bucketName, None, option=option, extensionHeaders=extensionHeaders
        )

    @funcCache
    def setBucketNotification(
        self, bucketName, notification=None, extensionHeaders=None
    ):
    ...
    def optionsObject(self, bucketName, objectKey, option, extensionHeaders=None):
        ...
        if option is not None:
            ...
            if option.get("accessControlRequestMethods") is not None:
                headers[const.ACCESS_CONTROL_REQUEST_METHOD_HEADER] = option[
                    "accessControlRequestMethods"
                ]
            if option.get("accessControlRequestHeaders") is not None:
                headers[const.ACCESS_CONTROL_REQUEST_HEADERS_HEADER] = option[
                    "accessControlRequestHeaders"
                ]
        return self._make_options_request(
            bucketName,
            objectKey,
            headers=headers,
            methodName="optionsBucket",
            extensionHeaders=extensionHeaders,
        )
    ...
    def getObjectMetadata(
        ...
        _requestHeaders = (
            requestHeaders[0]
            if isinstance(requestHeaders, list) and len(requestHeaders) == 1
            else requestHeaders
        )
        if _requestHeaders:
            headers[const.ACCESS_CONTROL_REQUEST_HEADERS_HEADER] = util.to_string(
                _requestHeaders
            )
        return self._make_head_request(
            bucketName,
            objectKey,
            pathArgs=pathArgs,
            headers=self.convertor._set_sse_header(
                sseHeader, headers=headers, onlySseCHeader=True
            ),
    ...
    def setObjectMetadata(
        ...
        return self._make_put_request(
            ...
            extensionHeaders=extensionHeaders,
            **self.convertor.trans_set_object_metadata(
                metadata=metadata, headers=headers, versionId=versionId
            )
        )
    ...
    def getObject(
        ...
        return self._make_get_request(
            ...
            readable=readable,
            extensionHeaders=extensionHeaders,
            **self.convertor.trans_get_object(
                getObjectRequest=getObjectRequest, headers=headers
            )
        )
    ...
    def _prepare_append_object_input(objectKey, headers, content):
        ...
        if headers.get("contentType") is None:
            headers["contentType"] = const.MIME_TYPES.get(
                objectKey[objectKey.rfind(".") + 1 :].lower()
            )

        return objectKey, headers, content

    def _prepare_file_notifier_and_entity(
        self, offset, file_size, headers, progressCallback, file_path, readable
    ):
        if offset is not None and 0 < offset < file_size:
            headers["contentLength"] = (
                headers["contentLength"]
                if 0 < headers["contentLength"] <= (file_size - offset)
            ...
            entity = util.get_entity_for_send_with_total_count(
                readable_object, totalCount, self.chunk_size, notifier
            )
        else:
            totalCount = headers["contentLength"]
            if totalCount > 0 and progressCallback is not None:
    ...
    def _prepare_content_notifier_and_entity(
        self,
        entity,
        headers,
        progressCallback,
        ...
        if entity is None:
            ...
            if headers.get("contentLength") is None:
                chunkedMode = True
                notifier = (
                    progress.ProgressNotifier(progressCallback, -1)
                    if progressCallback is not None
                    else progress.NONE_NOTIFIER
                )
                entity = util.get_readable_entity(
                    entity, self.chunk_size, notifier, autoClose
                )
            else:
                totalCount = util.to_long(headers.get("contentLength"))
                notifier = (
                    progress.ProgressNotifier(progressCallback, totalCount)
                    if totalCount > 0 and progressCallback is not None
                    else progress.NONE_NOTIFIER
                )
                entity = util.get_entity_for_send_with_total_count(
                    entity, totalCount, self.chunk_size, notifier, autoClose
                )
    ...
    def appendObject(
        ...
        objectKey, headers, content = self._prepare_append_object_input(
            objectKey, headers, content
        )
        ...
        if content.get("isFile"):
            ...
            if headers.get("contentType") is None:
                headers["contentType"] = const.MIME_TYPES.get(
                    file_path[file_path.rfind(".") + 1 :].lower()
                )
            ...
            headers["contentLength"] = (
                ...
                and headers["contentLength"] <= file_size
                else file_size
            )
            offset = util.to_long(content.get("offset"))
            headers, readable, notifier, entity = (
                self._prepare_file_notifier_and_entity(
                    offset, file_size, headers, progressCallback, file_path, readable
                )
            )
            headers = self.convertor.trans_put_object(
                metadata=metadata, headers=headers
            )
            self.log_client.log(DEBUG, "send Path:%s" % file_path)
        else:
            entity = content.get("content")
            entity, readable, chunkedMode, notifier = (
                self._prepare_content_notifier_and_entity(
                    entity,
                    headers,
                    progressCallback,
                    autoClose,
        ...
        try:
            if notifier is not None:
                notifier.start()
            ret = self._make_post_request(
                bucketName,
                objectKey,
                pathArgs={
                    "append": None,
                    "position": (
                        util.to_string(content["position"])
                        if content.get("position") is not None
                        else 0
                    ),
                },
                headers=headers,
    ...
    def putFile(
        ...
        if _flag:
            ...
            for f in os.listdir(file_path):
                ...
                if not objectKey:
                    key = util.safe_trans_to_gb2312(
                        "{0}/".format(os.path.split(file_path)[1]) + f
                    )
                else:
        ...
        totalCount = (
            util.to_long(headers["contentLength"])
            if headers.get("contentLength") is not None
    ...
    def _putFileHandleHeader(headers, size, objectKey, file_path):
        ...
        if headers.get("contentLength") is not None:
            headers["contentLength"] = (
                size if headers["contentLength"] > size else headers["contentLength"]
            )
    ...
    def _get_part_size(partSize, file_size, offset):
        partSize = (
            partSize
            if partSize is not None and 0 < partSize <= (file_size - offset)
            else file_size - offset
        )
        return partSize

    def _prepare_headers(
        self, md5, isAttachMd5, file_path, partSize, offset, sseHeader, headers
    ):
        if md5:
            headers[const.CONTENT_MD5_HEADER] = md5
        elif isAttachMd5:
            headers[const.CONTENT_MD5_HEADER] = util.base64_encode(
                util.md5_file_encode_by_size_offset(
                    file_path, partSize, offset, self.chunk_size
                )
            )
    ...
    def uploadPart(
        ...
        notifier = None
        if isFile:
            checked_file_part_info = self._check_file_part_info(
                content, offset, partSize
            )

            headers = {
                const.CONTENT_LENGTH_HEADER: util.to_string(
            ...
            headers = self._prepare_headers(
                md5,
                isAttachMd5,
                checked_file_part_info["file_path"],
                checked_file_part_info["partSize"],
            ...
            readable, notifier = self._prepare_upload_part_notifier(
                checked_file_part_info["partSize"], progressCallback, readable
            )
            ...
            if (
                ...
                if partSize is None:
                    self.log_client.log(
                        DEBUG, "missing partSize when uploading a readable stream"
                    )
                    chunkedMode = True
                    notifier = self._get_notifier_without_size(progressCallback)
                    entity = util.get_readable_entity(
                        content, self.chunk_size, notifier, autoClose
                    )
                else:
                    headers[const.CONTENT_LENGTH_HEADER] = util.to_string(partSize)
                    totalCount = util.to_long(partSize)
                    notifier = self._get_notifier_with_size(
                        progressCallback, totalCount
                    )
                    entity = util.get_entity_for_send_with_total_count(
                        content, totalCount, self.chunk_size, notifier, autoClose
                    )
            else:
                entity = content
                if entity is None:
    ...
    def _uploadPartWithNotifier(
        ...
        if isFile:
            ...
            headers[const.CONTENT_LENGTH_HEADER] = util.to_string(
                checked_file_part_info["partSize"]
            )
            headers = self._prepare_headers(
                md5,
            ...
            readable_object.seek(checked_file_part_info["offset"])
            entity = util.get_entity_for_send_with_total_count(
                readable_object, partSize, self.chunk_size, notifier
            )
        else:
            if (
                content is not None
                ...
                if partSize is None:
                    chunkedMode = True
                    entity = util.get_readable_entity(
                        content, self.chunk_size, notifier
                    )
                else:
                    headers[const.CONTENT_LENGTH_HEADER] = util.to_string(partSize)
                    entity = util.get_entity_for_send_with_total_count(
                        content, util.to_long(partSize), self.chunk_size, notifier
                    )
            else:
                entity = content
                if entity is None:
    ...
    def copyObject(
        ...
        return self._make_put_request(
            ...
            extensionHeaders=extensionHeaders,
            **self.convertor.trans_copy_object(
                metadata=metadata,
                headers=headers,
                versionId=versionId,
    ...
    def setObjectAcl(
        ...
        return self._make_put_request(
            ...
            objectKey,
            extensionHeaders=extensionHeaders,
            **self.convertor.trans_set_object_acl(
                acl=acl, versionId=versionId, aclControl=aclControl
            )
        )

    @funcCache
    def getObjectAcl(
        self, bucketName, objectKey, versionId=None, extensionHeaders=None
    ):
        pathArgs = {"acl": None}
        if versionId:
            pathArgs[const.VERSION_ID_PARAM] = util.to_string(versionId)
    ...
    def deleteObjects(self, bucketName, deleteObjectsRequest, extensionHeaders=None):
        ...
        return self._make_post_request(
            ...
            extensionHeaders=extensionHeaders,
            **self.convertor.trans_delete_objects(
    ...
    def restoreObject(
        ...
        return self._make_post_request(
            ...
            objectKey,
            extensionHeaders=extensionHeaders,
            **self.convertor.trans_restore_object(
                days=days, tier=tier, versionId=versionId
            )
        )
    ...
    def initiateMultipartUpload(
        ...
        if contentType is None:
            contentType = const.MIME_TYPES.get(
                objectKey[objectKey.rfind(".") + 1 :].lower()
            )
        ...
        return self._make_post_request(
            ...
            extensionHeaders=extensionHeaders,
            **self.convertor.trans_initiate_multipart_upload(
                acl=acl,
    ...
    def copyPart(
        ...
        return self._make_put_request(
            ...
            methodName="copyPart",
            extensionHeaders=extensionHeaders,
            **self.convertor.trans_copy_part(
    ...
    def completeMultipartUpload(
        ...
        self._assert_not_null(
            completeMultipartUploadRequest, "completeMultipartUploadRequest is empty"
        )
    ...
    def abortMultipartUpload(
        self, bucketName, objectKey, uploadId, extensionHeaders=None
    ):
        self._assert_not_null(uploadId, "uploadId is empty")
        return self._make_delete_request(
            bucketName,
    ...
    def getBucketStoragePolicy(self, bucketName, extensionHeaders=None):
        return self._make_get_request(
            bucketName,
            methodName="getBucketStoragePolicy",
            extensionHeaders=extensionHeaders,
    ...
    def setBucketEncryption(
        self, bucketName, encryption, key=None, extensionHeaders=None
    ):
        self._assert_not_null(encryption, "encryption is empty")
        return self._make_put_request(
            bucketName,
            pathArgs={"encryption": None},
    ...
    def deleteBucketEncryption(self, bucketName, extensionHeaders=None):
        return self._make_delete_request(
            bucketName, pathArgs={"encryption": None}, extensionHeaders=extensionHeaders
        )
    ...
    def bindBucketAlias(self, bucketName, aliasInfo=None, extensionHeaders=None):
        if aliasInfo is None:
            raise Exception("aliasInfo is None")
        self._assert_not_null(
            aliasInfo.get("alias"), "bucket alias should not be empty"
        )
        return self._make_put_request(
            bucketName,
            extensionHeaders=extensionHeaders,
    ...
    def deleteBucketAlias(self, bucketAlias, extensionHeaders=None):
        ...
        return self._make_delete_request(
            bucketName=bucketAlias, pathArgs=pathArgs, extensionHeaders=extensionHeaders
        )
    ...
    def unbindBucketAlias(self, bucketName, extensionHeaders=None):
        ...
        return self._make_delete_request(
            bucketName, pathArgs=pathArgs, extensionHeaders=extensionHeaders
        )
    ...
    def listBucketAlias(self, extensionHeaders=None):
        ...
        return self._make_get_request(
            pathArgs=pathArgs,
            methodName="ListBucketAlias",
            extensionHeaders=extensionHeaders,
        )

    @funcCache
    def createVirtualBucket(
        self,
        regionId,
        token,
        bucketName1,
        ...
        if cbaResp.status != 200:
            self._clear_virtual_bucket(
                const.VIRTUAL_BUCKET_CREATEBUCKET_STAGED,
                bucketName1,
                bucketName2,
                bucketAlias,
            )
        ...
        if bindResp.status != 200:
            self._clear_virtual_bucket(
                const.VIRTUAL_BUCKET_CREATEALIAS_STAGED,
                bucketName1,
                bucketName2,
                bucketAlias,
            )
        ...
        if bindResp.status != 200:
            self._clear_virtual_bucket(
                const.VIRTUAL_BUCKET_BINDALIAS_STAGED,
                bucketName1,
                bucketName2,
                bucketAlias,
            )
        ...
        replicationResp = self._set_virtual_replication(
            agencyName, bucketName1, bucketName2
        )
        ...
        replicationResp = self._set_virtual_replication(
            agencyName, bucketName2, bucketName1
        )
        if replicationResp.status != 200:
            self._clear_virtual_bucket(
                const.VIRTUAL_BUCKET_BINDALIAS_STAGED,
        ...
        return {
            "code": "OK",
            "message": "create virtual bucket success",
            "virtualBucketName": bucketAlias,
    ...
    def _get_cluster_group_id(self, azResp):
        azJsonResp = (
            util.jsonLoadsForPy2(azResp.body)
            if const.IS_PYTHON2
            else json.loads(azResp.body)
        )
        ...
        if not firstAZCgId:
            raise Exception(
                "this AZs first cluster group id is None, AZ key: %s" % firstAZKey
            )
        ...
        if not secondAZCgId:
            raise Exception(
                "this AZs first cluster group id is None, AZ key: %s" % secondAZKey
            )

        return firstAZCgId, secondAZCgId

    def _create_bucket_with_cluster_id(
        self, firstAZCgId, secondAZCgId, bucketName1, bucketName2, bucketAlias, header
    ):
        # 判断桶是否存在
        ...
        if bucket1CgId:
            ...
            cbResp = self.createBucket(
                bucketName2, header=header, extensionHeaders=extensionHeaders
            )
            if cbResp.status != 200:
                ...
                raise Exception(
                    "create bucket failed, cluster group id: %s, resp: %s"
        ...
        if bucket2CgId:
            ...
            cbResp = self.createBucket(
                bucketName1, header=header, extensionHeaders=extensionHeaders
            )
            if cbResp.status != 200:
                self._clear_virtual_bucket(
                    const.VIRTUAL_BUCKET_CREATEBUCKET_STAGED,
    ...
    def _clear_virtual_bucket(self, staged, bucketName1, bucketName2, bucketAlias):
        ...
        if staged >= const.VIRTUAL_BUCKET_CREATEALIAS_STAGED:
            ...
            if deleteAliasResp.status != 204:
                raise Exception(
                    "delete bucket alias failed, resp: %s" % deleteAliasResp
                )
    ...
    def downloadFile(
        ...
        return self._downloadFileWithNotifier(
            bucketName,
            objectKey,
            downloadFile,
            partSize,
    ...
    def getBucketFetchPolicy(self, bucketName, extensionHeaders=None):
        ...
        return self._make_get_request(
            bucketName,
            const.FETCH_POLICY_KEY,
            methodName="getBucketFetchPolicy",
            headers=headers,
            extensionHeaders=extensionHeaders,
        )
