...
class LDAPConnection:
    def __init__(self, url, baseDN="", dstIp=None):
        ...
        LOG.debug(
            "Connecting to %s, port %d, SSL %s" % (targetHost, self._dstPort, self._SSL)
        )
        try:
            af, socktype, proto, _, sa = socket.getaddrinfo(
                targetHost, self._dstPort, 0, socket.SOCK_STREAM
            )[0]
    ...
    def login(
        ...
        if authenticationChoice == "simple":
            ...
            type3, exportedSessionKey = getNTLMSSPType3(
                negotiate, bytes(type2), user, password, domain, lmhash, nthash
            )
            bindRequest["authentication"]["sicilyResponse"] = type3.getData()
            response = self.sendReceive(bindRequest)[0]["protocolOp"]
        else:
            raise LDAPSessionError(
                errorString="Unknown authenticationChoice: '%s'" % authenticationChoice
            )
                ...
                if responseControls is not None:
                    for responseControl in responseControls:
                        if str(requestControl["controlType"]) == CONTROL_PAGEDRESULTS:
                            if (
                                str(responseControl["controlType"])
                                == CONTROL_PAGEDRESULTS
                            ):
                                if hasattr(responseControl, "getCookie") is not True:
                                    responseControl = decoder.decode(
                                        encoder.encode(responseControl),
                                        asn1Spec=KNOWN_CONTROLS[CONTROL_PAGEDRESULTS](),
                                    )[0]
        ...
        while len(data) > 0:
            try:
                ...
                if message["messageID"] == 0:  # unsolicited notification
                    name = (
                        message["protocolOp"]["extendedResp"]["responseName"]
                        or message["responseName"]
                    )
                    notification = KNOWN_NOTIFICATIONS.get(
                        name, "Unsolicited Notification '%s'" % name
                    )
                    ...
                    raise LDAPSessionError(
                        ...
                        % (
                            notification,
                            message["protocolOp"]["extendedResp"][
                                "resultCode"
                            ].prettyPrint(),
    ...
    def _compileSimpleFilter(attribute, operator, value):
        ...
        if operator == ":=":  # extensibleMatch
            match = RE_EX_ATTRIBUTE_1.match(attribute) or RE_EX_ATTRIBUTE_2.match(
                attribute
            )
            if not match:
                raise LDAPFilterInvalidException(
            ...
            if value == "*" and operator == "=":  # present
                ...
                if assertions[0]:
                    substrings.append(
                        choice.clone().setComponentByName("initial", assertions[0])
                    )
                for assertion in assertions[1:-1]:
                    substrings.append(
                        choice.clone().setComponentByName("any", assertion)
                    )
                if assertions[-1]:
                    substrings.append(
                        choice.clone().setComponentByName("final", assertions[-1])
                    )
                ...
                raise LDAPFilterInvalidException(
                    "invalid filter '(%s%s%s)'" % (attribute, operator, value)
                )
