...
class LDAPSearch(object):
    ...
    def search_by_sid(self, sid, attributes=None):
        ...
        if attributes is None:
            attributes = ["cn"]
        self.con.search(
            self.domain_dn, "(ObjectSID={sid})".format(sid=sid), attributes=attributes
        )
    ...
    def search_admins(self):
        ...
        self.con.search(
            self.domain_dn,
            "(&(adminCount=1)(objectclass=person))",
        ...
        if self.con.result["result"] == 0:
            ...
            for en in entries:
                cn_name = en["cn"][0]
                sid = en["objectSid"][0]
                admin_users.append({"user": cn_name, "sid": sid, "domain": self.domain})
            return admin_users
        else:
            raise LDAPSearchFailException()
    ...
    def search_domain_controller(self):
        ...
        self.con.search(
            dn,
            "(&(objectCategory=computer)(userAccountControl:1.2.840.113556.1.4.803:=532480))",
    ...
    def get_support_aes_account(self):
        ...
        self.con.search(
            dn,
            "(&(objectClass=Computer)(msds-supportedencryptiontypes>=8))",
    ...
    def search_by_custom(self, filter_condition, attributes, paged_size=10000):
        dn = self.domain_dn
        self.con.search(
            dn, filter_condition, attributes=attributes, paged_size=paged_size
        )
        if self.con.result["result"] == 0 and len(self.con.entries) > 0:
            return self.con.entries
        elif self.con.result["result"] != 0:
    ...
    def search_admins_info(self, attributes_value, filter_query=""):
        ...
        entry_generator = self.con.extend.standard.paged_search(
